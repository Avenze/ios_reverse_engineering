...
local UI = GameTableDefine.InstanceRewardUI
local ResourceManger = GameTableDefine.ResourceManger
---@class InstanceRewardUIView:UIBaseView
local InstanceRewardUIView = Class("InstanceRewardUIView", UIView)


function InstanceRewardUIView:ctor()
    self.super:ctor()

    self.m_level = 1 -- 当前操作的彩蛋等级
    self.m_index = 0 -- 当前等级操作的彩蛋在某一级的索引
...
function InstanceRewardUIView:OnEnter()
...
function InstanceRewardUIView:InitView()
    ...
    for i = 1, Tools:GetTableSize(reallyRewards) do
        local rewardData = reallyRewards[i]
        if rewardData.rewardList then
            self.m_RewardDataList[rewardData.type] = rewardData
            self.m_level = math.max(self.m_level,rewardData.type)
        end
    end
    self.m_index = #reallyRewards[#reallyRewards].rewardList
    -- 点击开蛋事件:
...
function InstanceRewardUIView:SetRewardInfo()
    self.m_rewardFeel:PlayFeedbacks()

    local rewardID = self.m_RewardDataList[self.m_level].rewardList[self.m_index][1]
    local rewardData = self.m_uiModel.shopConfig[rewardID]

    if rewardData.type == 9 then
        local resType = ResourceManger:GetShopCashType(rewardData.country)
        local icon = ResourceManger:GetShopCashIcon(resType,rewardData.icon)
        self:SetText(self.m_rewardGO,"icon/num","x"..rewardData.amount.."H")
        self:SetSprite(self:GetComp(self.m_rewardGO,"icon","Image"),"UI_Shop",icon)
    else
        self:SetText(self.m_rewardGO,"icon/num","x"..rewardData.amount)
...
function InstanceRewardUIView:ShowAchievements()
    self.m_openingBtn.interactable = false
    self.m_tipGO:SetActive(false)
...
function InstanceRewardUIView:OpenAllRewardList()
                    ...
                    if shopCfg.type == 9 then -- 主场景货币特殊处理和批逻辑,不使用和批类型判断
                        if coinIndex == 0 then
                            ...
                            if resType == "euro" then
                                ...
                                GameTableDefine.InstanceFlyIconManager:StorageFlyIcon(6)
                            else
                                GameTableDefine.InstanceFlyIconManager:StorageFlyIcon(2)
                            end
                            local tmpData = {}
                            tmpData.amount = v
                            tmpData.icon = icon
                            ...
                            newAllRewardData[coinIndex].amount = newAllRewardData[coinIndex].amount + v
                        end
                    else
                        local tmpData = {}
                        tmpData.amount = v
    ...
    for k, v in ipairs(newAllRewardData) do
        if goCounter == 1 then
            ...
            if v.shopCfg and v.shopCfg.type == 9 then
                self:SetText(itemGO, "bg/num", "x"..tostring(v.amount).."H")
            else
                self:SetText(itemGO, "bg/num", "x"..tostring(Tools:SeparateNumberWithComma(v.amount)))
            end
        else
            local tempGo = UnityHelper.CopyGameByGo(itemGO, parentGO)
            self:SetSprite(self:GetComp(tempGo, "icon", "Image"), "UI_Shop", v.icon)
            if v.shopCfg and v.shopCfg.type == 9 then
                self:SetText(tempGo, "bg/num", "x"..tostring(v.amount).."H")
            else
                self:SetText(tempGo, "bg/num", "x"..tostring(Tools:SeparateNumberWithComma(v.amount)))
            end
        end
    ...
    if diamondIndex~=0 then
        GameTableDefine.InstanceFlyIconManager:StorageFlyIcon(3)
    end
end
...
function InstanceRewardUIView:OpenAEgg(isPre)
    local eggType = nil
    if self.m_index >= 1 then
        if not isPre then
        ...
        for i = self.m_level - 1, 1,-1 do
            if self.m_RewardDataList[i] then
                if not isPre then
                    self.m_level = i
                    self.m_index = #self.m_RewardDataList[i].rewardList
                end
                eggType = self.m_level
                break
            end
...
function InstanceRewardUIView:OnExit()
    if self.tiemr then
