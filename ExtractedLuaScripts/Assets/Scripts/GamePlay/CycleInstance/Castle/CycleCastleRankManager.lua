...
function CycleCastleRankManager:GMOpenRankSystem()
    self.enable = true
end
...
function CycleCastleRankManager:Init(saveData)
    -- 禁用排行榜
    if true then
        print("禁用排行榜")
        return
    ...
    if not self.saveData.CastleRankData then
        self.saveData.CastleRankData = {}
    end
    
    -- 作弊标识
    if not self.saveData.CastleRankData.cheat_tag then
        self.saveData.CastleRankData.cheat_tag = false
    end

    -- 上次请求时间
    if not self.saveData.CastleRankData.latest_request_time then
        self.saveData.CastleRankData.latest_request_time = 0
    end

    -- 奖励领取时间
    if not self.saveData.CastleRankData.claim_award_time then
        self.saveData.CastleRankData.claim_award_time = 0
    end

    -- 机器人随机名字存档
    if not self.saveData.CastleRankData.robot_names then
        self.saveData.CastleRankData.robot_names = {}
    end

    -- 服务器积分存档
    if not self.saveData.CastleRankData.latest_server_score then
        self.saveData.CastleRankData.latest_server_score = 0
    end

    if self.saveData.CastleRankData.cheat_tag and not self.saveData.CastleRankData.server_cheat_tag then
...
function CycleCastleRankManager:IsInit()
    return self.isInit
end

function CycleCastleRankManager:GetInstanceId()
    return self.instance_id
end

--- 进入排行榜界面
function CycleCastleRankManager:EnterCycleCastleInstanceRankUI()
    IsInCycleInstanceRankUI = true
    self:GetReportRank(2)
end

--- 退出排行榜界面
function CycleCastleRankManager:ExistCycleCastleInstanceRankUI()
    IsInCycleInstanceRankUI = false
end

--- 进入副本界面
function CycleCastleRankManager:EnterCycleCastleInstanceMainUI()
...
--- 退出副本界面
function CycleCastleRankManager:ExistCycleCastleInstanceMainUI()
...
--- 进入副本引导界面
function CycleCastleRankManager:EnterCycleCastleInstance()
    IsInCycleInstanceUI = true
    self:GetReportRank(1)
end

--- 退出副本引导界面
function CycleCastleRankManager:ExistCycleCastleInstance()
    IsInCycleInstanceUI = false
end

--- 当前分组scene_id 100、200、300、400、500、600
function CycleCastleRankManager:GetRankSceneId()
    local replace_str = "group_member_" .. self.instance_id .. "_" .. self.instance_index .. "_"
    local group_key_string = string.gsub(self.saveData.CastleRankData.group_key, replace_str, "")
...
--- 当前副本内分组
function CycleCastleRankManager:GetRankGroupKey()
    if self.saveData and self.saveData.CastleRankData and self.saveData.CastleRankData.group_key and self:GetRemainTime() > 0 then
        --local replace_str = "group_member_" .. self.instance_id .. "_" .. self.instance_index .. "_"
        --local group_key_string = string.gsub(self.saveData.CastleRankData.group_key, replace_str, "")
        --group_key_string = string.gsub(group_key_string, "00", "")

        local group_key_table = Tools:SplitString(self.saveData.CastleRankData.group_key, "_")
...
--- 当前副本内排名
function CycleCastleRankManager:GetRankNum()
    if self.saveData and self.saveData.CastleRankData and self.saveData.CastleRankData.rank_num and self:GetRemainTime() > 0 then
        return self.saveData.CastleRankData.rank_num
    end
...
--- 返回拉霸机历史获得代币数量
function CycleCastleRankManager:GetUserHistorySlotCoin()
    if self.saveData and self.saveData.CastleRankData and GameTableDefine.CycleInstanceDataManager:GetCurrentModel().GetHistorySlotCoin and self:GetRemainTime() > 0 then
...
---获得当前里程碑
function CycleCastleRankManager:GetCurInstanceKSLevel()
    return GameTableDefine.CycleInstanceDataManager:GetCurrentModel():GetCurInstanceKSLevel()
end

---排行奖励内容
function CycleCastleRankManager:GetRankReward()
    local reward_list = {}
    if self:HadClaimAward() then
        return reward_list
    end
    
    local award_list = self:GetAwardList(self.saveData.CastleRankData.rank_num)
...
---排行奖励详细内容
function CycleCastleRankManager:GetRankRewardDetail()
    local reward_list = {}
    if self:HadClaimAward() then
        return reward_list
    end
    ...
    self.saveData.CastleRankData.claim_award_time = os.time()
    return reward_list
end

---获得奖励列表
function CycleCastleRankManager:GetAwardList(rank_num)
    local award_list = {}
...
function CycleCastleRankManager:GetInstanceState()
    local state = 0
...
function CycleCastleRankManager:GetRemainTime(only_active)
...
---获得活动结束时间
function CycleCastleRankManager:GetInstanceEndTime()
    local remain_time_sec = 0
    if InstanceDataManager:GetInstanceIsActive() or InstanceDataManager:GetInstanceRewardIsActive() then
...
---排行奖励是否领取
function CycleCastleRankManager:HadClaimAward()
    return self.saveData and self.saveData.CastleRankData and self.saveData.CastleRankData.claim_award_time and self.saveData.CastleRankData.claim_award_time > 0
end

-- 处理一下机器人的分数，5的倍数
function CycleCastleRankManager:FormatRobotSocre(score)
...
-- 名字合集
function CycleCastleRankManager:RobotNameConf()
    local name_conf = {}
...
-- 获取机器人名字
function CycleCastleRankManager:GetRobotName(robot_id)
    if self.saveData.CastleRankData.robot_names and self.saveData.CastleRankData.robot_names[tostring(robot_id)] then
        return self.saveData.CastleRankData.robot_names[tostring(robot_id)]
    end
    ...
    self.saveData.CastleRankData.robot_names[tostring(robot_id)] = new_name
    return new_name
end

---合并本地机器人数据
function CycleCastleRankManager:MergeRobot(group_key, rank_data, group_created_at)
    local player_datas = {}
    ...
    local rank_num = 1
    self.saveData.CastleRankData.rank_data = {}
    for i, user_data in pairs(player_datas) do
        ...
        self.saveData.CastleRankData.rank_data[rank_num] = user_data
        if user_data.user_id == self.player_id then
            self.saveData.CastleRankData.rank_num = rank_num
            self.saveData.CastleRankData.user_rank_data = user_data
        end
...
--- 刷新 排行榜数据
function CycleCastleRankManager:DealRankData(data)
    --if not self.saveData.CastleRankData.group_key and data.group_key then
    if data.group_key then -- 分组标识，覆盖 以服务器为准，不然排行会跟服务器不一致
        self.saveData.CastleRankData.group_key = data.group_key
    end

    if data.group_created_at then -- 分组创建时间，覆盖 以服务器为准，不然排行会跟服务器不一致
        self.saveData.CastleRankData.group_created_at = data.group_created_at
    end

    if data.rank_data then -- 服务器返回数据
        self.saveData.CastleRankData.server_rank_data = data.rank_data
        -- 合并本地机器人数据
...
-- 使用存档数据进行界面数据的更新
function CycleCastleRankManager:UseOldData()
    local old_group_key = self.saveData.CastleRankData.group_key
    local old_rank_data = self.saveData.CastleRankData.server_rank_data
    local old_group_created_at = self.saveData.CastleRankData.group_created_at
...
--- 刷新排行榜排名
function CycleCastleRankManager:RefreshRankNum()
    if not self.enable then
        return
    end
    
    --print("Rank Log: 刷新排行榜排名...")
    EventManager:DispatchEvent(GameEventDefine.RefreshCastleRankNum, self.saveData.CastleRankData.rank_num)
end

--- 刷新排行榜数据
function CycleCastleRankManager:RefreshRankData()
    if not self.enable then
        return
    end
    ...
    EventManager:DispatchEvent(GameEventDefine.RefreshCastleRankData, self.saveData.CastleRankData.rank_data)
end

--- 玩家排行信息
function CycleCastleRankManager:GetUserRankInfo()
    --print("Rank Log: 玩家排行信息...")
    return self.saveData.CastleRankData.user_rank_data
end

--- 上报检查
function CycleCastleRankManager:ReportCheck()
    if not self.isInit then
        return false
    end
    
    if self.saveData.CastleRankData.cheat_tag then
    ...
    local score = self:GetUserHistorySlotCoin()
    if score > self.saveData.CastleRankData.latest_server_score then
        print("Rank Log: 上报检查, 积分有变化, 可上报: ", score, self.saveData.CastleRankData.latest_server_score)
        return true
    end
...
---上报排行榜
---@param refresh_rank --是否需要刷新排名
---@param refresh_data --是否需要刷新数据
function CycleCastleRankManager:ReportRank(trigger_type)
    local score = self:GetUserHistorySlotCoin()
    ...
    GameServerHttpClient:ReportRank(self.instance_id, self.instance_index, score, function(ret)
        ...
        if ret.errno == 0 then
            self.saveData.CastleRankData.latest_request_time = os.time()
            self.saveData.CastleRankData.latest_server_score = score -- 更新最近上报分数，分组信息存储
            local skinId, hatId, bagId = GameServerHttpClient:GetUserDressUp()
            self.saveData.CastleRankData.latest_server_skin_id = skinId
            self.saveData.CastleRankData.latest_server_hat_id = hatId
            self.saveData.CastleRankData.latest_server_bag_id = bagId
            self.saveData.CastleRankData.latest_server_instance_state = self:GetInstanceState()
...
-- 请求服务器排行榜数据
function CycleCastleRankManager:GetServerReportRank(trigger_type)
...
---获取排行榜
---@return table
function CycleCastleRankManager:GetReportRank(trigger_type)
    if not self:ReportCheck() then
        return
    end

    local score = self:GetUserHistorySlotCoin()
    if score > self.saveData.CastleRankData.latest_server_score then -- 积分增加了，上报，拉取最新数据
        print("Rank Log: 积分增加了，上报玩家数据，拉取最新数据...")
        self:ReportRank(trigger_type)
    else
        -- 上次请求失败，直接拉取最新数据
        if not LastRequestState or self.saveData.CastleRankData.latest_server_instance_state ~= self:GetInstanceState() then
            print("Rank Log: 上次请求失败 或 副本状态改变，直接拉取最新数据...")
            self:GetServerReportRank(trigger_type)
            return
        end

        if trigger_type and os.time() - self.saveData.CastleRankData.latest_request_time <= playerRefreshRate then
            print("Rank Log: 频繁摘取数据, 使用存档数据...")
            self:UseOldData()
        else
            local skinId, hatId, bagId = GameServerHttpClient:GetUserDressUp()
            if skinId ~= self.saveData.CastleRankData.latest_server_skin_id or hatId ~= self.saveData.CastleRankData.latest_server_hat_id or bagId ~= self.saveData.CastleRankData.latest_server_bag_id then
...
--  2. 单次钻石变化量>6万，视为作弊用户
function CycleCastleRankManager:CheatCheck(old_diamond, change_value)
    ...
    if self.isInit and not self.saveData.CastleRankData.server_cheat_tag then
        self.saveData.CastleRankData.cheat_tag = true
        self:NoticeServerCheat()
    end
end

function CycleCastleRankManager:NoticeServerCheat()
    GameServerHttpClient:CheatTag(self.instance_id, self.instance_index, function(ret)
        if ret and ret.errno == 0 then
            self.saveData.CastleRankData.server_cheat_tag = true
        end
    end)
end
...
---获取玩家是否为作弊玩家
---@return boolean
function CycleCastleRankManager:CheatTag()
    return self.saveData.CastleRankData.cheat_tag
end
