...
local CycleCastleUnlockUIView = Class("CycleCastleUnlockUIView", UIView)

function CycleCastleUnlockUIView:ctor()
    self.super:ctor()
    self.m_data = {}
    self.m_roomID = nil
end

function CycleCastleUnlockUIView:OnEnter()
    self:SetButtonClickHandler(self:GetComp("RootPanel/QuitBtn","Button"), function()
        self:DestroyModeUIObject()
    end)
end

function CycleCastleUnlockUIView:OnExit()
    self.super:OnExit(self)

    local currentModel = CycleInstanceDataManager:GetCurrentModel() ---@type CycleCastleModel
    --还原摄像机位置
    local cameraFocus = self:GetComp("SceneObjectRange", "CameraFocus")
    currentModel:LookAtSceneGO(self.m_roomID, 1, cameraFocus, true)
end

function CycleCastleUnlockUIView:InitView(id)
...
return CycleCastleUnlockUIView