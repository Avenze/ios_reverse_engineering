...
---@class CycleCastlePopUIView:UIBaseView 礼包界面
local CycleCastlePopUIView = Class("CycleCastlePopUIView", UIView)

CycleCastlePopUIView.ShopTypeToItemNamePath = {
    [3]  = "bg/Num_Diamond",
    [26] = "bg/Num_Exp",
    [27] = "bg/Num_Skill",
    [30] = "bg/Num_SlotCoin",
...
function CycleCastlePopUIView:ctor()
    self.super:ctor()
...
function CycleCastlePopUIView:OnEnter()
    ...
    ShopManager:refreshBuySuccess(function(shopId)
        ShopManager:Buy(shopId, false, function()
            ...
            if clickShopID == shopId then
                ...
                for _, content in ipairs(packConfig.content_shop_id) do
                    ...
                    if childCfg.type == 27 then --获得的技能书
                        for k,v in pairs(rewardInfo) do
                            if v>0 then
                                table.insert(res,{itemType = 27,str = v,icon = "icon_cy2_skill_"..k})
                            end
                        end
                ...
                GameTableDefine.CycleCastleMainViewUI:RefreshPackButton()
            end
        end,
        function()
...
function CycleCastlePopUIView:RefreshGiftPackItem(index,tran)
    index = index + 1
    ...
    self:GetGo(go,"title/" .. packCfg.gift_name):SetActive(true)
    self:SetText(go,"title/" .. packCfg.gift_name .. "/1", nameStr)
    self:SetText(go,"title/" .. packCfg.gift_name .. "/2", nameStr)
    self:SetText(go,"title/" .. packCfg.gift_name .. "/3", nameStr)


    --Icon 设置了icon才改变
    local iconImage = self:GetGo(go, "title/token/" .. packCfg.gift_token, "Image")
    ...
    while i<100 do
        ...
        if shopInfo then
            ...
            if curShopCfg.type == 23 then
                self:InitShopItem(self:GetOrCreateItem(parentGO,i),1466,1)
                self:InitShopItem(self:GetOrCreateItem(parentGO,i+1),1467,1)
                self:InitShopItem(self:GetOrCreateItem(parentGO,i+2),1468,1)
                i = i + 2
            else
...
function CycleCastlePopUIView:GetOrCreateItem(parentGO,index)
...
function CycleCastlePopUIView:InitShopItem(itemGO,shopID,complex)
    ...
    for k,v in pairs(CycleCastlePopUIView.ShopTypeToItemNamePath) do
...
function CycleCastlePopUIView:OnLeftButtonDown()
...
function CycleCastlePopUIView:OnRightButtonDown()
...
---滑动停靠,更新index
function CycleCastlePopUIView:OnListSnap(index)
...
--function CycleCastlePopUIView:ShowGiftPop(shopID)
...
function CycleCastlePopUIView:OnExit()
