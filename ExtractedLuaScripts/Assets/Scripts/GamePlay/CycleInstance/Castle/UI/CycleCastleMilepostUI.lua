...
function CycleCastleMilepostUI:GetView()
    self.m_view = GameUIManager:SafeOpenUI(ENUM_GAME_UITYPE.CYCLE_CASTLE_MILEPOST_UI, self.m_view, require("GamePlay.CycleInstance.Castle.UI.CycleCastleMilepostUIView"), self, self.CloseView)
    return self.m_view
end

function CycleCastleMilepostUI:CloseView()
    GameUIManager:CloseUI(ENUM_GAME_UITYPE.CYCLE_CASTLE_MILEPOST_UI)
    self.m_view = nil
    collectgarbage("collect")
end


function CycleCastleMilepostUI:Refresh(scrollIndex,scrollToNewest)
...
function CycleCastleMilepostUI:CalculateData()
    local data = {}
    local curData = {}
...
function CycleCastleMilepostUI:GetRewardStatusByCfg(itemCfg)
    local result = 1
    --TODO:获取当前的里程碑经验
...
function CycleCastleMilepostUI:ShowMilepostPanel(scrollIndex)
