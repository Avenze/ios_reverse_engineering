...
local CycleCastleMilepostUIView = Class("CycleCastleMilepostUIView", UIView)

local UnityHelper = CS.Common.Utils.UnityHelper

function CycleCastleMilepostUIView:ctor()
    self.super:ctor()
    self.m_data = {}
    self.m_notClaimGO = nil ---当前未获取奖励的ListItemGO, 用来给引导用
end

function CycleCastleMilepostUIView:OnEnter()
    ...
    self.tiemr = GameTimer:CreateNewTimer(1,function()
        ...
        if proValue >= 1 then
            GameTableDefine.CycleCastleMilepostUI:Refresh()
            if self.tiemr then
...
function CycleCastleMilepostUIView:OnPause()

end

function CycleCastleMilepostUIView:OnResume()
end

function CycleCastleMilepostUIView:OnExit()
    if self.tiemr then
        GameTimer:_RemoveTimer(self.tiemr)
    end
end

function CycleCastleMilepostUIView:ShowMilepostPanel(data, curData, index,scrollToNewest)
    -- local cur = {}
    --     --TODO:待实装内容
    ...
    if not index then
        if scrollToNewest then
            ...
            --从奖励界面返回界面时判断引导
            if not currentModel:IsGuideCompleted(14027) then
                --开启引导
                GameTableDefine.GuideManager.currStep = 14027
                GameTableDefine.GuideManager:ConditionToStart()
                currentModel:SetGuideCompleted(14027)
            end
        end
    else
        self.m_list:ScrollTo(scrollIndex)
...
function CycleCastleMilepostUIView:UpdateMilepostRewardList(index, tran)
...
function CycleCastleMilepostUIView:OpenRewardInfo(curGo, rewardInfoData)
...
function CycleCastleMilepostUIView:GetNotClaimedRewardBtn()
...
function CycleCastleMilepostUIView:GetComp(obj, child, uiType)
    if obj == "GetNotClaimedRewardBtn" then
        return self:GetNotClaimedRewardBtn()
    else
        return self:getSuper(CycleCastleMilepostUIView).GetComp(self,obj, child,uiType)
    end
end

return CycleCastleMilepostUIView