...
function CycleCastleBuildingUI:GetView()
    --print("打开副本建筑UI \n"..debug.traceback())
    self.m_model = {}
    self.m_view = GameUIManager:SafeOpenUI(ENUM_GAME_UITYPE.CYCLE_CASTLE_BUILDING_UI, self.m_view, require("GamePlay.CycleInstance.Castle.UI.CycleCastleBuildingUIView"), self, self.CloseView)
    return self.m_view
end

function CycleCastleBuildingUI:CloseView()
    --停止播放BGM
    SoundEngine:StopSFX(currentBGM)

    GameUIManager:CloseUI(ENUM_GAME_UITYPE.CYCLE_CASTLE_BUILDING_UI)
    self.m_view = nil
    collectgarbage("collect")
end

---房间升级流程
function CycleCastleBuildingUI:DoFactoryUpgradeProcess(roomID,furIndex)
    ...
    --1.隐藏主界面
    GameTableDefine.CycleCastleMainViewUI:GuideTimeUIState(false)
    ...
    if decorationFurID then
        ...
        if decorationGO then
            GameTimer:CreateNewTimer(0.5,function()
                --2.在0.5s后,视角锁定到装饰
                local cameraFocus = GameTableDefine.CycleCastleMainViewUI.m_view:GetComp("RoomLevel_SceneRange","CameraFocus")
                currentScene:LookAtGameObject(room.roomGO.roomBox,cameraFocus,false,function()
                    ...
                    GameTimer:CreateNewTimer(length,function()
                        ...
                        GameTimer:CreateNewTimer(1.2,function()
                            ...
                            GameTableDefine.CycleCastleMainViewUI:GuideTimeUIState(true)
...
function CycleCastleBuildingUI:ShowFactoryUI(roomID, furIndex)
...
function CycleCastleBuildingUI:RefreshView()
    if self.m_view then
...
--显示补给建筑UI
function CycleCastleBuildingUI:ShowSupplyBuildingUI(roomID, furIndex)
...
--显示码头UI
function CycleCastleBuildingUI:ShowWharfUI(roomID, furIndex)
