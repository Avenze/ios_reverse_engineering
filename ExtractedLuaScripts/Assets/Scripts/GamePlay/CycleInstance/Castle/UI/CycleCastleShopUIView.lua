...
---@class CycleCastleShopUIView:UIBaseView
local CycleCastleShopUIView = Class("CycleCastleShopUIView", UIView)

function CycleCastleShopUIView:ctor()
    self.super:ctor()
    self.m_data = {}
    self.m_dataSize = {}
    self.typeToIndex = {}
...
function CycleCastleShopUIView:OnEnter()
...
function CycleCastleShopUIView:OnExit()
    --购买成功事件 反注册
...
function CycleCastleShopUIView:InitList()
...
function CycleCastleShopUIView:UpdateSaleData()
    local shopFrameCfg = {}
...
--对列表的每个item的单独刷新
function CycleCastleShopUIView:UpdateListItem(i, tran)
    local index = i + 1
...
function CycleCastleShopUIView:ShowList(data, refresh)
    self:CleanTimer()
    self.m_data = data
    if refresh then
        self.m_dataSize = {}
...
function CycleCastleShopUIView:RefreshDiamond()
...
function CycleCastleShopUIView:CleanTimer()
    
end

function CycleCastleShopUIView:SetFrame1Details(go, data)
...
function CycleCastleShopUIView:SetFrame3Details(go, data)
...
function CycleCastleShopUIView:SetFrame2Details(go, data)
...
function CycleCastleShopUIView:SetFrame4Details(go, data)
    local itemsName = {}
...
function CycleCastleShopUIView:SetFramePesoDetails(go, data)
...
function CycleCastleShopUIView:CheckDataAndGameObject(temp, data, index)
...
function CycleCastleShopUIView:SetTempItemDetails(centGo, rootData, data)
    -- 特殊处理免费领取的代币商品
    if data.isFree then
        if self.freeGetTimer then
            GameTimer:StopTimer(self.freeGetTimer)
...
function CycleCastleShopUIView:ShowItemDetails(rootData, data, valueShow)
...
function CycleCastleShopUIView:CloseItemDetaild()
...
function CycleCastleShopUIView:EnterToSpecial()
    if self.m_list then
...
--检测是否能购买对应等级的礼包
function CycleCastleShopUIView:CheckCanGetLvlGift(shopID)
    local resultID = 0
...
function CycleCastleShopUIView:TurnPage(index)
    if self.__timers["TurnPage"] then
        GameTimer:StopTimer(self.__timers["TurnPage"])
        self.__timers["TurnPage"] = nil
    end
...
---显示购买确认界面
function CycleCastleShopUIView:ShowConfirmPanel(shopConfig,confirmCallback)
...
---隐藏购买确认界面
function CycleCastleShopUIView:HideConfirmPanel()
    local frame1Panel = self:GetGoOrNil("frame1_info")
    if frame1Panel then
