...
---@class CycleCastleSlotMachineUI
...
---每一行有多少个格子
CycleCastleSlotMachineUI.SlotCount = 3

--CycleCastleSlotMachineUI.RewardName = {
--    [1] = "红色",
--    [2] = "蓝色",
--    [3] = "绿色"
--}
...
---奖励类型
CycleCastleSlotMachineUI.RewardType = {
    AAA = 1,
    BBB = 2,
    CCC = 3
}
...
---奖励出现个数对应的奖励倍率
CycleCastleSlotMachineUI.RewardMagnificationByCount = {
    [1] = 0,
    [2] = 4,
    [3] = 12,
    [4] = 32,
...
--CycleCastleSlotMachineUI.RewardList = {
--    [1] = CycleCastleSlotMachineUI.RewardType.AAA,
--    [2] = CycleCastleSlotMachineUI.RewardType.BBB,
--    [3] = CycleCastleSlotMachineUI.RewardType.CCC
--}

CycleCastleSlotMachineUI.RewardCount = 3

--- 1.技能书，2.英雄经验，3.货币，4.积分
CycleCastleSlotMachineUI.FlyIconConfig = {
    [1] = "RootPanel/reward1",
    [2] = "RootPanel/reward2",
    [3] = "RootPanel/reward3",
    [4] = "RootPanel/reward4",
...
CycleCastleSlotMachineUI.CurRewards = {}
CycleCastleSlotMachineUI.CurRewardsNotCombine = {} ---不合并的奖励


---[奖励倍率,硬币倍率]
CycleCastleSlotMachineUI.SlotMagnificationConfig = {
    [1] = {1,1},
    [2] = {3,3},
    [3] = {10,7}
}
...
function CycleCastleSlotMachineUI:ctor()
...
function CycleCastleSlotMachineUI:GetView()
    self.m_view = GameUIManager:SafeOpenUI(ENUM_GAME_UITYPE.CYCLE_CASTLE_SLOT_MACHINE, self.m_view, require("GamePlay.CycleInstance.Castle.UI.CycleCastleSlotMachineUIView"), self, self.CloseView)
    return self.m_view
end

function CycleCastleSlotMachineUI:OpenView()
    self:Init()
    self:GetView():Invoke("Init")
end

function CycleCastleSlotMachineUI:CloseView()
    GameUIManager:CloseUI(ENUM_GAME_UITYPE.CYCLE_CASTLE_SLOT_MACHINE)
    self.m_view = nil
...
function CycleCastleSlotMachineUI:SetConfigsToSlot(level)
    level = level or 1
    ...
    CycleCastleSlotMachineUI.SlotCount = #self.m_slotMachineConfigs[1].compose
    CycleCastleSlotMachineUI.RewardCount = #self.m_slotMachineConfigs

    for i = 1, CycleCastleSlotMachineUI.SlotCount do
...
---初始化配置
function CycleCastleSlotMachineUI:Init()
    ...
    if not self.m_initialized then
        --硬币倍率
        CycleCastleSlotMachineUI.SlotMagnificationConfig[3][2] = math.floor(CycleCastleSlotMachineUI.SlotMagnificationConfig[3][1] * (1-0.01 * ConfigMgr.config_global.ten_fold_discount))
        self.m_initialized = true
    end
end

---获取对应位置的图标
function CycleCastleSlotMachineUI:GetSlotSpriteName(slotIndex,itemIndex)
...
function CycleCastleSlotMachineUI:GetRandomResultType(slotIndex, type)
    ...
    --等概率随机一个结果
    --return CycleCastleSlotMachineUI.RewardList[math.random(1, #CycleCastleSlotMachineUI.RewardList)]
end

---拉动遥感，提前算出奖励，再通过UI表现让玩家获得
function CycleCastleSlotMachineUI:Push()
    local slotResult = {}
    local guideID = GameTableDefine.GuideManager.currStep
    if guideID == 14034 or guideID == 14035 then
        slotResult[1] = 3
        slotResult[2] = 1
        slotResult[3] = 2
        ...
        if resultCfg == nil then
            ...
            --填充剩下的格子
            if resultNum < CycleCastleSlotMachineUI.SlotCount then
                local remaining = {} 
                for k, v in pairs(self.m_slotMachineRewardsConfig.rewards) do
                ...
                if CycleCastleSlotMachineUI.SlotCount - resultNum == 1 then
    ...
    CycleCastleSlotMachineUI.CurRewards = rewards
    CycleCastleSlotMachineUI.CurRewardsNotCombine = rewardsNotCombine
    ...
    local coinRatio = CycleCastleSlotMachineUI.SlotMagnificationConfig[magIndex][1] ---代币倍率
...
---获取奖励
function CycleCastleSlotMachineUI:GetRewards()
    --local rewardString = "最终奖励为"
    local finalRewardCount = 0
    local magIndex = GameTableDefine.CycleInstanceDataManager:GetCurrentModel():GetCurSlotMagnification()
    local coinRatio = CycleCastleSlotMachineUI.SlotMagnificationConfig[magIndex][1] ---代币倍率
    --计算奖励
    for rewardID,rewardCount in pairs(CycleCastleSlotMachineUI.CurRewards) do
        ---奖励倍率
        local rewardRatio = CycleCastleSlotMachineUI.RewardMagnificationByCount[rewardCount] * coinRatio
        ...
        if rewardRatio > 0 then
            ...
            ShopManager:Buy_LimitPackReward(shopID , nil,function()
                ...
                --flyIcon
                local flyIconPath = CycleCastleSlotMachineUI.FlyIconConfig[rewardID]
                local showValue = ShopManager:SetValueToShow(value, cfg)
                if typeName == "instanceSkillBook" then
                    self.m_view:Invoke("PlayFlyIconSkill",flyIconPath,rewardCount==CycleCastleSlotMachineUI.SlotCount,skillInfos)
                else
                    self.m_view:Invoke("PlayFlyIcon",flyIconPath,rewardCount==CycleCastleSlotMachineUI.SlotCount,showValue)
                end
    ...
    if finalRewardCount == 0 then
        ...
        ShopManager:Buy(shopID , false,nil,function()
            ...
            --flyIcon
            local flyIconPath = CycleCastleSlotMachineUI.FlyIconConfig[3]
...
function CycleCastleSlotMachineUI:GetUpgradeCondition()
...
---拉霸机最大等级
---@return number
function CycleCastleSlotMachineUI:GetMaxLevel()
...
function CycleCastleSlotMachineUI:MaybeBigReward()
    local firstReward = CycleCastleSlotMachineUI.CurRewardsNotCombine[1] or 0
    for i = 2, CycleCastleSlotMachineUI.SlotCount-1 do
        if CycleCastleSlotMachineUI.CurRewardsNotCombine[i] ~= firstReward then
            return false
        end
    end
    return true
...
return CycleCastleSlotMachineUI