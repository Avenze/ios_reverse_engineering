...
---@class CycleCastleTaskUIView:UIBaseView
local CycleCastleTaskUIView = Class("CycleCastleTaskUIView", UIView)
...
function CycleCastleTaskUIView:ctor()
...
function CycleCastleTaskUIView:OnEnter()
    ...
    for i = 1, TASK_ITEM_COUNT do
        ...
        self:SetButtonClickHandler(taskUIItem.jumpBtn,function()
            if CycleInstanceDataManager:GetInstanceState(CycleInstanceDataManager.instance_id) ~= CycleInstanceDataManager.instanceState.isActive then
                GameTableDefine.CycleCastleSlotMachineUI:CloseView()
...
function CycleCastleTaskUIView:OnExitBtnDown()
    self:DestroyModeUIObject()
end

function CycleCastleTaskUIView:OnExit()
...
function CycleCastleTaskUIView:RewardBtnDown(taskUIItem)
    ...
    if getReward then
        ...
        if taskConfig.taskShowType == 1 then
            self:RefreshTaskInfo(taskUIItem,CycleCastleTaskManager:GetType1TaskData())
        else
...
-----@param taskUIItem TaskUIItem
function CycleCastleTaskUIView:JumpBtnDown(taskUIItem)
...
function CycleCastleTaskUIView:RefreshTaskInfo(taskUIItem,taskData)
    taskData = taskData or taskUIItem.taskData
    local taskID = taskData.taskID or 0
    local taskConfig = CycleCastleTaskManager:GetTaskConfigByTaskID(taskID)
    ...
    if taskConfig then
        ...
        local descStr,haveStr,needStr,isComplete = CycleCastleTaskManager:GetTaskShowValues(taskID)
        ...
        --2024-11-26fy添加容错
        if taskUIItem and taskUIItem.isCompleteMark then
...
function CycleCastleTaskUIView:Init()
    local taskData = CycleCastleTaskManager:GetTaskData()
...
function CycleCastleTaskUIView:RefreshAllTaskInfo()
    --1特殊处理,如果1完成了就显示1，没完成就找个完成的显示
    self:RefreshTaskInfo(self.m_taskUIItems[1],CycleCastleTaskManager:GetType1TaskData())
...
---跳转到对应需要解锁的
function CycleCastleTaskUIView:LocateRoom(roomID,furIndex)
        ...
        if curRoomData.state == 0 then
            ...
            self.m_currentModel:LookAtSceneGO(roomID,nil,nil,nil,function()
                ...
                if self.m_currentModel:RoomIsUnlock(unlock_room) then
                    CycleCastleUnlockUI:ShowUI(roomID)
                end
            ...
            self.m_currentModel:LookAtSceneGO(roomID,furIndex,nil,nil,function()
                ...
                if curRoomCfg then
                    if curRoomCfg.room_category == 1 then
                        CycleCastleBuildingUI:ShowFactoryUI(roomID,furIndex)
                    elseif curRoomCfg.room_category == 2 or curRoomCfg.room_category == 3 then
                        CycleCastleBuildingUI:ShowSupplyBuildingUI(roomID,furIndex)
                    else
...
function CycleCastleTaskUIView:LocateFurniture(taskConfig)
...
---跳转到任务目标
---@param taskUIItem TaskUIItem
function CycleCastleTaskUIView:JumpToTaskTarget(taskUIItem)
    local taskConfig = taskUIItem.taskConfig
    if taskConfig then
        --CycleCastleTaskManager:FakeSetTaskAsComplete(taskConfig.id)
        ...
        elseif taskConfig.task_type == 5 then
            if not GameTableDefine.CycleCastleSlotMachineUI.m_view then
                GameTableDefine.CycleCastleSlotMachineUI:OpenView()
            end
        end
        if taskConfig.task_type ~= 5 then
            if GameTableDefine.CycleCastleSlotMachineUI.m_view then
                GameTableDefine.CycleCastleSlotMachineUI:CloseView()
            end
        end
