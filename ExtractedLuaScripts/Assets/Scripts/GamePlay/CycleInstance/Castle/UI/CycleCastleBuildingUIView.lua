...
local CashNotEnoughColor = UnityHelper.GetColor("#EC1418")
local ExpNotEnoughColor = UnityHelper.GetColor("#FE1313")
local GlintingTime = 0.35

---@class CycleCastleBuildingUIView :UIBaseView
local CycleCastleBuildingUIView = Class("CycleCastleBuildingUIView", UIView)

function CycleCastleBuildingUIView:ctor()
    self.super:ctor()
    self.m_data = {}
    ...
    self.m_currentInstanceModel = nil ---@type CycleCastleModel
    self.m_workerIconImage = nil ---@type UnityEngine.UI.Image
    self.m_workerIconGOList = nil ---@type UnityEngine.GameObject[]
    --家具
...
function CycleCastleBuildingUIView:OnEnter()
    ...
    GameTableDefine.CycleCastleMainViewUI:SetEventActive(false)
    --GameTableDefine.CycleCastleMainViewUI:SetPackActive(false)
end

function CycleCastleBuildingUIView:OnExit()
    --GameTableDefine.CycleCastleMainViewUI:SetPackActive(true)
    GameTableDefine.CycleCastleMainViewUI:SetEventActive(true)
...
function CycleCastleBuildingUIView:RefreshRoomInfo()
    ...
    --员工Icon
    local employeeIcon = self:GetComp( self.progressEmployee,"","Image")
    self:SetSprite(employeeIcon,"UI_Common",roomCfg.worker_exist)
    --说话的员工Icon
    ...
    if roomCfg.room_category == 1 then
        ...
        if roomCfg.worker_exist then
            ...
            self:SetSprite(self.m_employeeTempOnImage,"UI_Common",roomCfg.worker_exist)
            self:SetSprite(self.m_employeeTempOffImage,"UI_Common",roomCfg.worker_absent)
        end
...
---刷新家具等级有关联的信息
function CycleCastleBuildingUIView:RefreshFurnitureInfo()
    ...
    self:SetSprite(self.m_heroProductionIconImage, "UI_Common", productionCfg.icon)
    if not nextFurLevelCfg then
        self.progressBonus:SetActive(false)
    else
        self.progressBonusNumText.text = BigNumber:FormatBigNumberSmall(BigNumber:Subtract(nextFurLevelCfg.product,currentFurLevelCfg.product))
    end
    ...
    --判断是否是最高收入
    local _, highestRoomID, _ = self.m_currentInstanceModel:GetCurHighestProfit()
    if roomID == highestRoomID and self.m_currentInstanceModel:GetSellingRoomID() ~= roomID then
        --self.m_personTalkText.text = GameTextLoader:ReadText("TXT_INSTANCE_WORKER_TALK_1")
        self.m_haveTalkContent = true
        self:PlayTextReveal("TXT_INSTANCE_WORKER_TALK_1")
    end
...
---播放打字效果
function CycleCastleBuildingUIView:PlayTextReveal(txtID)
...
function CycleCastleBuildingUIView:UpdatePersonTalk(forceHide)
...
---房间升级进度条和按钮的状态
function CycleCastleBuildingUIView:RefreshRoomUpgradeInfo()
    --房间升级进度条
    local curFurLevelID = self.factoryFurData[tostring(selectIndex)].id --当前选中的家具levelID
    local currentFurLevelCfg = furnitureLevelConfig[curFurLevelID] --当前选中的家具levelConfig
...
---员工栏
function CycleCastleBuildingUIView:RefreshWorkerUI()
    local curFurLevelID = self.factoryFurData[tostring(selectIndex)].id --当前选中的家具levelID
    local currentFurLevelCfg = furnitureLevelConfig[curFurLevelID] --当前选中的家具levelConfig
    local currentFurCfg = furnitureConfig[currentFurLevelCfg.furniture_id]
    --员工数据
    local workerMax = currentFurCfg.seat
    local curWorker = self.m_currentInstanceModel:GetWorkerNum(self.roomID)
    -- 员工栏
    --Tools:SetTempGo(self.employeeItem, workerMax, true, function(go, index)
    --    self:GetGo(go, "on"):SetActive(index <= curWorker)
    --end)
    if not self.m_workerIconGOList then
        self.m_workerIconGOList = {}
        Tools:SetTempGo(self.employeeItem, workerMax, true, function(go, index)
            --self:GetGo(go, "on"):SetActive(index <= curWorker)
            self.m_workerIconGOList[index] = go
        end)
    end
    for i = 1, workerMax do
        self:GetGo(self.m_workerIconGOList[i], "on"):SetActive(i <= curWorker)
    end
end

---房间是否需要升级，满足升级房间的家具等级条件
function CycleCastleBuildingUIView:RoomNeedUpgrade()
    local curFurLevelID = self.factoryFurData[tostring(selectIndex)].id --当前选中的家具levelID
    local currentFurLevelCfg = furnitureLevelConfig[curFurLevelID] --当前选中的家具levelConfig
    local currentFurCfg = furnitureConfig[currentFurLevelCfg.furniture_id]
    local nextFurLevelCfg = self.m_currentInstanceModel:GetFurlevelConfig(currentFurCfg.id, currentFurLevelCfg.level + 1)
...
function CycleCastleBuildingUIView:OnFurUpgradeBtnClicked(showResNotEnoughTip)
    local curFurLevelID = self.factoryFurData[tostring(selectIndex)].id --当前选中的家具levelID
    local currentFurLevelCfg = furnitureLevelConfig[curFurLevelID] --当前选中的家具levelConfig
    local currentFurCfg = furnitureConfig[currentFurLevelCfg.furniture_id]
    local nextFurLevelCfg = self.m_currentInstanceModel:GetFurlevelConfig(currentFurCfg.id, currentFurLevelCfg.level + 1)
    ...
    --员工数量是否改变
    if currentFurLevelCfg.worker ~= nextFurLevelCfg.worker then
        self:RefreshWorkerUI()
    end
    ...
    GameTableDefine.CycleCastlePopUI:PackTrigger(5, self.roomID)
    
    return true
end

function CycleCastleBuildingUIView:ShowFactoryUI(roomID, select, isFirst)
    ...
    if isFirst then
        ...
        self:CheckNewRoomGuide()
    end
end

function CycleCastleBuildingUIView:ShowSupplyBuildingUI(roomID, select, isFirst)
    isOpen = isFirst
    if select then
        selectIndex = select
    else
...
function CycleCastleBuildingUIView:UpdateSupplyBuildingFurItem(index,trans)
    index = index + 1
...
function CycleCastleBuildingUIView:ShowWharfUI(roomID,select,isFirst)
    isOpen = isFirst
    if select then
        selectIndex = select
    else
...
function CycleCastleBuildingUIView:UpdateWharfFurItem(index,trans)
    index = index + 1
    local go = trans.gameObject
...
function CycleCastleBuildingUIView:GetFurnitureMaxlevel(furnitureID)
...
function CycleCastleBuildingUIView:SelectFurniture(index,furData)
...
--region 英雄
---刷新英雄信息
function CycleCastleBuildingUIView:RefreshHero()
    local roomCfg = roomsConfig[self.roomID]
    local heroID = roomCfg.hero_id
    local heroData = CycleCastleHeroManager:GetHeroData(heroID)
    local heroLevel = heroData.level
    local heroConfig = CycleCastleHeroManager:GetHeroConfig(heroID,heroLevel)
    ...
    if CycleCastleHeroManager:IsMaxLevel(heroID) then
        ...
        local heroNextConfig = CycleCastleHeroManager:GetHeroConfig(heroID,heroLevel+1)
        ...
        if not isEnough then
            GameTableDefine.CycleCastlePopUI:PackTrigger(2)
        end
    end
end

---英雄升级按钮
function CycleCastleBuildingUIView:OnHeroUpgradeBtnDown(showResNotEnoughTip)
    local roomCfg = roomsConfig[self.roomID]
    local heroID = roomCfg.hero_id
    local result,nextLevelCfg = CycleCastleHeroManager:UpgradeHero(heroID)
    if result == CycleCastleHeroManager.HeroUpgradeResult.Success then
        self:RefreshHero()
        self:RefreshFurnitureInfo()
        CycleCastleMainViewUI:Refresh()
        return true
    elseif result == CycleCastleHeroManager.HeroUpgradeResult.ResNotEnough then
        --if showResNotEnoughTip then
        --    ChooseUI:Choose("TXT_INSTANCE_SHOP_TIP",function()
        --        GameTableDefine.CycleCastleShopUI:GetView()
        --    end)
        --end
    elseif result == CycleCastleHeroManager.HeroUpgradeResult.LevelMax then
        --满级
    end
end
--endregion
...
function CycleCastleBuildingUIView:OnUpdate()
...
function CycleCastleBuildingUIView:InitUpgradePanel(isClickUpgradeBtn)
...
function CycleCastleBuildingUIView:RefreshRoomUpgradePanelDynamic()
    ...
    elseif roomData.needShowUpgradeAnim then
        ...
        self:DestroyModeUIObject()
        CycleCastleBuildingUI:DoFactoryUpgradeProcess(roomID,1)
...
function CycleCastleBuildingUIView:GetRoomUpgradeNodeList(roomID)
    local nodeList = {1}
    local index = 2
...
---刷新房间升级界面里显示的所有信息
function CycleCastleBuildingUIView:RefreshRoomUpgradePanel()
...
---按下房间升级界面中的升级按钮
function CycleCastleBuildingUIView:OnRoomUpgradeBtnDown()
    local roomData = self.m_currentInstanceModel:GetCurRoomData(self.roomID)
    --升级中
...
---按下房间升级跳过按钮
function CycleCastleBuildingUIView:OnUpgradeSkipBtnDown()
    local roomData = self.m_currentInstanceModel:GetCurRoomData(self.roomID)
...
---点击对话框
function CycleCastleBuildingUIView:OnPersonTalkBtnDown()
    local roomID = self.roomID
    local roomCfg = roomsConfig[self.roomID]
    ...
    if roomID == highestRoomID and self.m_currentInstanceModel:GetSellingRoomID() ~= roomID then
        ...
        self:DestroyModeUIObject(false,function()
            ...
            CycleInstanceDataManager:GetCurrentModel():GetScene():LookAtSceneGO(sellID,nil,nil,nil,function()
                GameTableDefine.CycleCastleSellUI:ShowWharfUI(sellID)
                GameTimer:CreateNewTimer(0.5,function()
                    GameUIManager:SetEnableTouch(true)
                    GuideManager.currStep = 14101
        ...
        if not BigNumber:CompareBig(storedResCount,needResCount) then
            ...
            self:DestroyModeUIObject(false,function()
                ...
                CycleInstanceDataManager:GetCurrentModel():GetScene():LookAtSceneGO(resRoomID,nil,nil,nil,function()
                    CycleCastleBuildingUI:ShowFactoryUI(resRoomID)
                    GameTimer:CreateNewTimer(0.5,function()
                        GameUIManager:SetEnableTouch(true)
                        GuideManager.currStep = 14201
                        GuideManager:ConditionToStart()
                    end)
                end)
            end)
...
---启动和更新家具闪烁
function CycleCastleBuildingUIView:StartGlinting()
...
---关闭家具闪烁
function CycleCastleBuildingUIView:StopGlinting()
    if not self.m_glintingTimer then
        return
    end
...
function CycleCastleBuildingUIView:RefreshFurLevelReward(isUpgrade)
    ...
    if isUpgrade then
        if currentFurLevelCfg.milePoint_node then
            ...
            FlyIconsUI:ShowCycleMilepost(currentIndex,"Instance2_Milepost_Fly",BigNumber:FormatBigNumber(currentFurLevelCfg.milePoint_node),function()
...
--function CycleCastleBuildingUIView:CheckRoomUpgradeGuide()
--    --1.是否是1号房间
...
---检查是否需要开启2号房间引导3
function CycleCastleBuildingUIView:CheckNewRoomGuide()
    --1.是否是2号房间
    if self.roomID ~= 1022 then
        return false
    end
    --2.是否触发过引导
    if self.m_currentInstanceModel:IsGuideCompleted(14050) then
        return false
    end
    --3.开启引导
    GuideManager.currStep = 14050
    GuideManager:ConditionToStart()
    self.m_currentInstanceModel:SetGuideCompleted(14050)
end

return CycleCastleBuildingUIView
