...
---@class CycleCastleClient:ActorBase
---@field super ActorBase
local CycleCastleClient = Class("CycleCastleClient", ActorBase)

function CycleCastleClient:ctor()
    self.super.ctor(self)
    self.initialized = false
    self.m_type = ActorDefine.ActorType.CycleCastleClient
    self.m_go = nil
    self.m_animator = nil ---@type UnityEngine.Animator
end

function CycleCastleClient:Init(id, go, actorData)
    ...
    if not self.m_stateMachine then
        ...
        self:TryTransState(function()
            if self.data.leader then
                self.m_stateMachine:ChangeState(ActorDefine.CycleInstanceState.CycleCastleClientFollow, self)
            else
                self.m_stateMachine:ChangeState(ActorDefine.CycleInstanceState.CycleCastleClientCome, self)
            end
        end)
    end
    self.super.Init(self, id, go, actorData, self.m_stateMachine)
    self.data.type = "CycleCastleClient"
    self.m_go = go
    self.initialized = true
    self.m_go.transform.localScale = Vector3(3, 3, 3)
    
    --self:SetDoorTrigger()
    print("创建 CycleCastleClient", id)
end

--设置气泡的显示
function CycleCastleClient:SetTheDisplayOfBubbles(typeB, interactiveRoomId)
    ...
    elseif typeB == "eat" then
        error("CycleCastleClient在此副本没有 eat 状态")
    elseif typeB == "sleep" then
        error("CycleCastleClient在此副本没有 sleep 状态")
    end
    state.speed = speed
    ...
    FloatUI:SetObjectCrossCamera(self, function(view)
        view:Invoke("RefreshCycleCastleClientBubble", self.typeB, self.bubbleState)
    end,function()
        if not self.view then
            return
        end
        self.view:Invoke("RefreshCycleCastleClientBubble")
    end,0)
end

--region 初始化
---设置与门交互的Trigger
function CycleCastleClient:SetDoorTrigger()
