...
---@class CycleInstanceRoom:BaseScene
local CycleInstanceRoom = Class("CycleInstanceRoom",CycleInstanceRoomBase)
...
function CycleInstanceRoom:DeclareVariable()
    self.roomType = {
    ...
    self.unlockFloat = nil --解锁房间的FloatUI
    self.needMatFloat = nil --缺少资源的FloatUI
    self.path = nil -- 上次的加载路径
end

function CycleInstanceRoom:Init(roomType,data,roomCfg,GO,cb)
    ...
    self.furConfig = self.m_currentModel.furnitureConfig
    self.furLevelConfig = self.m_currentModel.furnitureLevelConfig
    self:InitRoomGO()
    self:InitFurnitureGo()
end

function CycleInstanceRoom:OnEnter()
end

function CycleInstanceRoom:OnExit()
end

function CycleInstanceRoom:Update(dt)
end

function CycleInstanceRoom:OnPouse()
end

function CycleInstanceRoom:OnResume()
end

function CycleInstanceRoom:InitRoomGO()
    --构造物体索引表
    ...
    for k, v in pairs(self.roomConfig.furniture) do
        local furID = v.id
        local furConfig = self.m_currentModel:GetFurConfigByID(furID)
        local findIndex = 1
        ...
        if furConfig.seat and furConfig.seat > 0 then --如果是有子节点的家具
            local subCount = self.m_currentModel:GetAllFurSeatCount(furID)
            subFurList[k] = {}
            for i=1,subCount do
...
function CycleInstanceRoom:InitPortState()
    if self.curRoomType == 4 and self.roomData.state == 2 then
        ...
        --     if furData.state == 1 and furConfig.resource_type > 0 and furConfig.shipCD == 0 then
...
function CycleInstanceRoom:ShowRoom()
...
function CycleInstanceRoom:InitFurnitureGo()
    if self.roomData.state == 2 then
        self:ShowFurniture(nil, false)
    end
end

function CycleInstanceRoom:ShowFurniture(index, isBuy)
    if self.roomData.state == 2 then
        if index then
...
function CycleInstanceRoom:ShowFurPrefab(levelID,index,isBuy)
    -- 显示模型,如果没有就先加载再显示
...
--加载完成后的处理
function CycleInstanceRoom:LoadFinishHandle(GO,index,levelID,isBuy)
    ...
    if furConfig.seat and furConfig.seat > 0 then
        local subFurCount = self.m_currentModel:GetAllFurSeatCount(furConfig.id)
    ...
    if isBuy then
        self.m_currentModel:ShowSelectFurniture(parent,false)
        --播放物品购买动画
...
function CycleInstanceRoom:ShowBubble(state)
    ...
    if state == 1 then
        ...
        if self.unlockFloat == nil then
            self.unlockFloat = {["go"] = go, m_type = "room"}
            FloatUI:SetObjectCrossCamera(self.unlockFloat, function(view)
                if view then
                    --print("显示修建房屋FLoatUI",self.unlockFloat.guid)
                    view:Invoke("ShowCycleIslandBuildingUnlockButtton", curRoomConfig, timeWait, self.unlockFloat)
                end
            end, nil, 0)
        else
        ...
        if self.needMatFloat == nil then
            self.needMatFloat = {["go"] = self.GORoot, m_type = "room"}
            FloatUI:SetObjectCrossCamera(self.needMatFloat, function(view)
                if view then
                    --print("显示修建房屋FLoatUI",self.unlockFloat.guid)
                    view:Invoke("ShowCycleIslandBuildingNeedMatTips", curRoomConfig.material[2] and curRoomConfig.material[1])
                end
            end, nil, 0)
        end
    end
...
function CycleInstanceRoom:HideBubble()
end

function CycleInstanceRoom:GetFurnitureGoByIndex(index)
...
function CycleInstanceRoom:GetSubFurnitureGoByIndex(index)
