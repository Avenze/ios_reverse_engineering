...
---@class CycleInstanceRewardUIView:UIBaseView
local CycleInstanceRewardUIView = Class("CycleInstanceRewardUIView", UIView)

function CycleInstanceRewardUIView:ctor()
    self.super:ctor()
    self.m_data = {}
    self.isShopUIRequire = false
end

function CycleInstanceRewardUIView:OnEnter()
    self:SetButtonClickHandler(self:GetComp("BgCover", "Button"), function()
        -- print("Close the CycleInstanceRewardUIView")
        self:DestroyModeUIObject()
    end)
    self.m_list = self:GetComp("RootPanel/milestonePanel/reward/list", "ScrollRectEx")
end

function CycleInstanceRewardUIView:OnPause()
end

function CycleInstanceRewardUIView:OnResume()
end

function CycleInstanceRewardUIView:OnExit()
...
function CycleInstanceRewardUIView:ShowGetReward(rewards,isShopUIRequires, scrollIndex)
    ...
    if shopCfg.type == 27 then
        ...
        for k,v in pairs(skillInfos) do
            if v>0 then
                ...
                self:SetSprite(self:GetComp(go,"icon", "Image"), "UI_Common", "icon_cy1_skill_"..k)
...
function CycleInstanceRewardUIView:ShowAllNotClaimRewardsGet(allRewards)
        ...
        if shopCfg then
            if shopCfg.type == 9 then
                ...
                if not tmpSameTypeRes[currentCoutry] then
                    ...
                    tmpSameTypeRes[currentCoutry].icon = shopCfg.icon
                else
...
function CycleInstanceRewardUIView:ShowGiftCodeGetRewards(allRewards, isUIShow)
    --进行显示了
    self.isShopUIRequire = isUIShow
    local goCounter = 1
    local itemGo = self:GetGoOrNil("background/MediumPanel/list/item")
    local parentGo = self:GetGoOrNil("background/MediumPanel/list")
    for k, v in pairs(allRewards or {}) do
        local curAltas = "UI_Shop"
        if v.icon_altas then
            curAltas = v.icon_altas
        end
        if goCounter == 1 then
            self:SetSprite(self:GetComp(itemGo, "icon", "Image"), curAltas, v.icon)
            self:SetText(itemGo, "bg/num", "+".. v.num)
        else
            local tmpGo = UnityHelper.CopyGameByGo(itemGo, parentGo)
            self:SetSprite(self:GetComp(tmpGo, "icon", "Image"), curAltas, v.icon)
            self:SetText(tmpGo, "bg/num", "+".. v.num)
        end
        goCounter  = goCounter + 1
    end
end

return CycleInstanceRewardUIView
