...
local CycleIslandOfflineRewardUIView = Class("CycleIslandOfflineRewardUIView", UIView)

function CycleIslandOfflineRewardUIView:ctor()
    self.super:ctor()
end

function CycleIslandOfflineRewardUIView:OnEnter()
    local disRewardData = {}
    ...
    self.rewardData = currentModel:GetCurInstanceOfflineRewardData()
    ...
    if self.rewardData.rewardMoney and self.rewardData.rewardMoney > 0 then
        ...
        local tempData = { ["itemType"] = 31, ["icon"] = "icon_cy1_cash", ["str"] = BigNumber:FormatBigNumber(math.floor(self.rewardData.rewardMoney)) }
        table.insert(disRewardData, tempData)
    end
    
    if self.rewardData.rewardExp and self.rewardData.rewardExp > 0 then
        local iconName = CycleInstanceDataManager:GetInstanceBind().cashIcon
        local tempData = { ["itemType"] = 26, ["icon"] = "icon_cy1_HeroExperience", ["str"] = BigNumber:FormatBigNumber(math.floor(self.rewardData.rewardExp)) }
        table.insert(disRewardData, tempData)
    end
    
    if self.rewardData.rewardPoint and self.rewardData.rewardPoint > 0 then
        local iconName = CycleInstanceDataManager:GetInstanceBind().cashIcon
        local tempData = { ["itemType"] = 201, ["icon"] = "icon_cy1_milePoint", ["str"] = BigNumber:FormatBigNumber(math.floor(self.rewardData.rewardPoint)) }
        table.insert(disRewardData, tempData)
    end
    ...
    local maxTime = currentModel:GetMaxOfflineTime()
    local curDisOfflineTime = currentModel:GetCurOfflineDisplayTime()
...
function CycleIslandOfflineRewardUIView:OnExit()
...
return CycleIslandOfflineRewardUIView