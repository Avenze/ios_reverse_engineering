...
function CycleIslandSellUI:GetView()
    print("打开副本码头UI \n"..debug.traceback())
    self.m_view = GameUIManager:SafeOpenUI(ENUM_GAME_UITYPE.CYCLE_ISLAND_SELL_UI, self.m_view, require("GamePlay/CycleInstance/Island/UI/CycleIslandSellUIView"), self, self.CloseView)
    return self.m_view
end

function CycleIslandSellUI:CloseView()
    --停止播放BGM
    SoundEngine:StopSFX(currentBGM)

    GameUIManager:CloseUI(ENUM_GAME_UITYPE.CYCLE_ISLAND_SELL_UI)
    self.m_view = nil
...
--显示码头UI
function CycleIslandSellUI:ShowWharfUI(roomID, furIndex)
