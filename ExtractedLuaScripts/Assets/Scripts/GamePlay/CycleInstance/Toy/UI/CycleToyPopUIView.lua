...
---@class CycleToyPopUIView:UIBaseView 礼包界面
local CycleToyPopUIView = Class("CycleToyPopUIView", UIView)

CycleToyPopUIView.ShopTypeToItemNamePath = {
    [3]  = "bg/Num_Diamond",
    [26] = "bg/Num_Exp",
    [27] = "bg/Num_Skill",
    [30] = "bg/Num_SlotCoin",
...
function CycleToyPopUIView:ctor()
    self.super:ctor()
...
function CycleToyPopUIView:OnEnter()
    ...
    ShopManager:refreshBuySuccess(function(shopId)
        ShopManager:Buy(shopId, false, function()
            ...
            if clickShopID == shopId then
                ...
                for _, content in ipairs(packConfig.content_shop_id) do
                    ...
                    if childCfg.type == 27 then --获得的技能书
                        for k,v in pairs(rewardInfo) do
                            if v>0 then
                                table.insert(res,{itemType = 27,str = v,icon = "icon_toy_skill_"..k})
                            end
                        end
                ...
                GameTableDefine.CycleToyMainViewUI:RefreshPackButton()
            end
        end,
        function()
...
function CycleToyPopUIView:RefreshGiftPackItem(index,tran)
    index = index + 1
    ...
    while i<100 do
        ...
        if shopInfo then
            ...
            if curShopCfg.type == 23 then
                self:InitShopItem(self:GetOrCreateItem(parentGO,i),1525,1)
                self:InitShopItem(self:GetOrCreateItem(parentGO,i+1),1526,1)
                self:InitShopItem(self:GetOrCreateItem(parentGO,i+2),1527,1)
                i = i + 2
            else
...
function CycleToyPopUIView:GetOrCreateItem(parentGO,index)
...
function CycleToyPopUIView:InitShopItem(itemGO,shopID,complex)
    ...
    for k,v in pairs(CycleToyPopUIView.ShopTypeToItemNamePath) do
...
function CycleToyPopUIView:OnLeftButtonDown()
...
function CycleToyPopUIView:OnRightButtonDown()
...
---滑动停靠,更新index
function CycleToyPopUIView:OnListSnap(index)
...
--function CycleToyPopUIView:ShowGiftPop(shopID)
...
function CycleToyPopUIView:OnExit()
