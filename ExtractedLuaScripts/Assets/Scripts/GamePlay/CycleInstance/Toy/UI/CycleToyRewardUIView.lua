...
local CycleToyModel = GameTableDefine.CycleInstanceDataManager:GetCurrentModel()

local CycleInstanceDataManager = GameTableDefine.CycleInstanceDataManager

---@class CycleToyRewardUIView:UIBaseView
local CycleToyRewardUIView = Class("CycleToyRewardUIView", UIView)

function CycleToyRewardUIView:ctor()
    self.super:ctor()
    self.m_data = {}
    self.isShopUIRequire = false
end

function CycleToyRewardUIView:OnEnter()
    self:SetButtonClickHandler(self:GetComp("BgCover", "Button"), function()
        -- print("Close the CycleToyRewardUIView")
        ...
        if not self.isShopUIRequire then
            ...
            if state == CycleInstanceDataManager.instanceState.isActive then
                if GameTableDefine.CycleToyMilepostUI.m_view then
                    GameTableDefine.CycleToyMilepostUI:Refresh(self.cacheIndex)
                end
            end
        end
    end)
...
function CycleToyRewardUIView:OnPause()
end

function CycleToyRewardUIView:OnResume()
end

function CycleToyRewardUIView:OnExit()
end

function CycleToyRewardUIView:ShowGetReward(rewards,isShopUIRequires, scrollIndex)
    ...
    if shopCfg.type == 27 then
        ...
        for k,v in pairs(skillInfos) do
            if v>0 then
                ...
                self:SetSprite(self:GetComp(go,"icon", "Image"), "UI_Common", "icon_toy_skill_"..k)
...
function CycleToyRewardUIView:ShowCEOCardsGet(rewards,isShopUIRequires, scrollIndex)
    self.isShopUIRequire = isShopUIRequires
    self.cacheIndex = scrollIndex
    if not rewards or not next(rewards) then
        self:DestroyModeUIObject()
    ...
    if itemGo then
        ...
        for i = 1, curChildCount do
            itemGo.transform.parent:GetChild(i - 1).gameObject:SetActive(false)
        end
...
function CycleToyRewardUIView:ShowAllNotClaimRewardsGet(allRewards)
...
function CycleToyRewardUIView:ShowAllNotClaimRewardsGetHaveCeoReward(allRewards, extendDiamond, ceoCards, normalKeys, premiumKeys)
     --step.1需要整合同样商品的id的数量
     local tmpShopItemRewards = {}
     local tmpSameTypeRes = {}
     for k, v in pairs(allRewards) do
