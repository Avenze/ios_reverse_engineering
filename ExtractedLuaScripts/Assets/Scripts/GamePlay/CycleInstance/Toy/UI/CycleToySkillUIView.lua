...
local CycleToySkillUIView = Class("CycleToySkillUIView", UIView)

function CycleToySkillUIView:ctor()
    self.super:ctor()
    self.container = {}
end

function CycleToySkillUIView:OnEnter()
...
function CycleToySkillUIView:OnPause()

end

function CycleToySkillUIView:OnResume()
    self:ShowSkillPanel(CycleToyModel:GetCurSkillData())
end

function CycleToySkillUIView:OnExit()

end

function CycleToySkillUIView:ShowSkillPanel(skillData)
    -- local tempIcon = "btn_premiumpack10"
    --TODO：根据数据封装的模块获取当前的技能点数
    local curSkills = CycleToyModel:GetCurSkillData()
    for i = 1, 3 do
        local panelGo = self:GetGoOrNil("RootPanel/"..i.."Panel")
        
        local curSkillData = skillData[i]
        local curSkillCfg = GameTableDefine.CycleToySkillUI:GetCurSkillItemCfg(curSkills[i])
        local nextSkillCfg = GameTableDefine.CycleToySkillUI:GetNextSkillItemCfg(curSkills[i])
        local isMax = false
        if not nextSkillCfg then
            isMax = true
        end
        if panelGo and curSkillCfg then
            local curSkillPoints = CycleToyModel:GetCurSkillPoints(curSkillCfg.skill_type)
            ...
            if upgradeBtn then
                ...
                self:SetButtonClickHandler(upgradeBtn, function()
                    ...
                    if isHaveEnoughPoint then
                        if CycleToyModel:UpdateSkill(curSkillCfg.id) then
                            self:ShowSkillPanel(CycleToyModel:GetCurSkillData())
                        end
                    else
                        ...
                        GameTableDefine.CycleToyPopUI:PackTrigger(3)
