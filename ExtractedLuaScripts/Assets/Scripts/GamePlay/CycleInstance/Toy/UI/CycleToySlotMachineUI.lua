...
---每一行有多少个格子
CycleToySlotMachineUI.SlotCount = 3

--CycleToySlotMachineUI.RewardName = {
--    [1] = "红色",
--    [2] = "蓝色",
--    [3] = "绿色"
--}
...
---奖励类型
CycleToySlotMachineUI.RewardType = {
    AAA = 1,
    BBB = 2,
    CCC = 3
}
...
---奖励出现个数对应的奖励倍率
CycleToySlotMachineUI.RewardMagnificationByCount = {
    [1] = 0,
    [2] = 4,
    [3] = 12,
    [4] = 32,
...
--CycleToySlotMachineUI.RewardList = {
--    [1] = CycleToySlotMachineUI.RewardType.AAA,
--    [2] = CycleToySlotMachineUI.RewardType.BBB,
--    [3] = CycleToySlotMachineUI.RewardType.CCC
--}

CycleToySlotMachineUI.RewardCount = 3

--- 1.技能书，2.英雄经验，3.货币，4.积分,5.钻石,6.蓝图
CycleToySlotMachineUI.FlyIconConfig = {
    [1] = "RootPanel/reward1",
    [2] = "RootPanel/reward2",
    [3] = "RootPanel/reward3",
    [4] = "RootPanel/reward4",
...
CycleToySlotMachineUI.CurRewards = {}
CycleToySlotMachineUI.CurRewardsNotCombine = {} ---不合并的奖励


---[奖励倍率,硬币倍率]
CycleToySlotMachineUI.SlotMagnificationConfig = {
    [1] = {1,1},
    [2] = {3,3},
    [3] = {10,7}
}
...
function CycleToySlotMachineUI:ctor()
...
function CycleToySlotMachineUI:GetView()
    self.m_view = GameUIManager:SafeOpenUI(ENUM_GAME_UITYPE.CYCLE_TOY_SLOT_MACHINE, self.m_view, require("GamePlay.CycleInstance.Toy.UI.CycleToySlotMachineUIView"), self, self.CloseView)
    return self.m_view
end

function CycleToySlotMachineUI:OpenView()
    self:Init()
    self:GetView():Invoke("Init")
end

function CycleToySlotMachineUI:CloseView()
    GameUIManager:CloseUI(ENUM_GAME_UITYPE.CYCLE_TOY_SLOT_MACHINE)
    self.m_view = nil
...
function CycleToySlotMachineUI:SetConfigsToSlot(level)
    level = level or 1
    ...
    CycleToySlotMachineUI.SlotCount = #self.m_slotMachineConfigs[1].compose
    CycleToySlotMachineUI.RewardCount = #self.m_slotMachineConfigs

    for i = 1, CycleToySlotMachineUI.SlotCount do
...
---初始化配置
function CycleToySlotMachineUI:Init()
    ...
    if not self.m_initialized then
        --硬币倍率
        CycleToySlotMachineUI.SlotMagnificationConfig[3][2] = math.floor(CycleToySlotMachineUI.SlotMagnificationConfig[3][1] * (1-0.01 * ConfigMgr.config_global.ten_fold_discount))
        self.m_initialized = true
    end
end

---获取对应位置的图标
function CycleToySlotMachineUI:GetSlotSpriteName(slotIndex,itemIndex)
...
function CycleToySlotMachineUI:GetRandomResultType(slotIndex, type)
    ...
    --等概率随机一个结果
    --return CycleToySlotMachineUI.RewardList[math.random(1, #CycleToySlotMachineUI.RewardList)]
end

---拉动遥感，提前算出奖励，再通过UI表现让玩家获得
function CycleToySlotMachineUI:Push()
    local slotResult = {}
    local guideID = GameTableDefine.GuideManager.currStep
    if guideID == 15215 then
        slotResult[1] = 3
        slotResult[2] = 1
        slotResult[3] = 3
        ...
        if resultCfg == nil then
            ...
            --填充剩下的格子
            if resultNum < CycleToySlotMachineUI.SlotCount then
                local remaining = {} 
                for k, v in pairs(self.m_slotMachineRewardsConfig.rewards) do
                ...
                if CycleToySlotMachineUI.SlotCount - resultNum == 1 then
    ...
    CycleToySlotMachineUI.CurRewards = rewards
    CycleToySlotMachineUI.CurRewardsNotCombine = rewardsNotCombine
    ...
    local coinRatio = CycleToySlotMachineUI.SlotMagnificationConfig[magIndex][1] ---代币倍率
...
---获取奖励
function CycleToySlotMachineUI:GetRewards()
    --local rewardString = "最终奖励为"
    local finalRewardCount = 0
    local magIndex = GameTableDefine.CycleInstanceDataManager:GetCurrentModel():GetCurSlotMagnification()
    local coinRatio = CycleToySlotMachineUI.SlotMagnificationConfig[magIndex][1] ---代币倍率
    --计算奖励
    for rewardID,rewardCount in pairs(CycleToySlotMachineUI.CurRewards) do
        ---奖励倍率
        local rewardRatio = CycleToySlotMachineUI.RewardMagnificationByCount[rewardCount] * coinRatio
        ...
        if rewardRatio > 0 then
            ...
            ShopManager:Buy_LimitPackReward(shopID , nil,function()
                ...
                --flyIcon
                local flyIconPath = CycleToySlotMachineUI.FlyIconConfig[rewardID]
                local showValue = ShopManager:SetValueToShow(value, cfg)
                if typeName == "instanceSkillBook" then
                    self.m_view:Invoke("PlayFlyIconSkill",flyIconPath,rewardCount==CycleToySlotMachineUI.SlotCount,skillInfos)
                else
                    self.m_view:Invoke("PlayFlyIcon",flyIconPath,rewardCount==CycleToySlotMachineUI.SlotCount,showValue)
                end
    ...
    if finalRewardCount == 0 then
        ...
        ShopManager:Buy(shopID , false,nil,function()
            ...
            --flyIcon
            local flyIconPath = CycleToySlotMachineUI.FlyIconConfig[3]
...
function CycleToySlotMachineUI:GetUpgradeCondition()
...
---拉霸机最大等级
---@return number
function CycleToySlotMachineUI:GetMaxLevel()
...
function CycleToySlotMachineUI:MaybeBigReward()
    local firstReward = CycleToySlotMachineUI.CurRewardsNotCombine[1] or 0
    for i = 2, CycleToySlotMachineUI.SlotCount-1 do
        if CycleToySlotMachineUI.CurRewardsNotCombine[i] ~= firstReward then
            return false
        end
    end
    return true
