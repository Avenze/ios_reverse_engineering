...
---@class CycleToyBuildingUIView :UIBaseView
local CycleToyBuildingUIView = Class("CycleToyBuildingUIView", UIView)

function CycleToyBuildingUIView:ctor()
    self.super:ctor()
    self.m_data = {}
    ...
    self.m_currentInstanceModel = nil ---@type CycleToyModel
...
function CycleToyBuildingUIView:OnEnter()
    ...
    --GameTableDefine.CycleToyMainViewUI:SetPackActive(false)
end

function CycleToyBuildingUIView:OnExit()
...
function CycleToyBuildingUIView:RefreshRoomInfo()
...
---刷新家具等级有关联的信息
function CycleToyBuildingUIView:RefreshFurnitureInfo()
    ...
    local data = CycleToyBluePrintManager:GetProductionDataByProductID(productionID)
    local level = data.starLevel or 0
    local bpConfig = CycleToyBluePrintManager:GetBPConfigByProductionLevel(productionID,level)
...
---播放打字效果
function CycleToyBuildingUIView:PlayTextReveal(txtID)
...
function CycleToyBuildingUIView:UpdatePersonTalk(forceHide)
...
---房间升级进度条和按钮的状态
function CycleToyBuildingUIView:RefreshRoomUpgradeInfo()
    --房间升级进度条
    local curFurLevelID = self.factoryFurData[tostring(selectIndex)].id --当前选中的家具levelID
    local currentFurLevelCfg = furnitureLevelConfig[curFurLevelID] --当前选中的家具levelConfig
...
---员工栏
function CycleToyBuildingUIView:RefreshWorkerUI()
    local curFurLevelID = self.factoryFurData[tostring(selectIndex)].id --当前选中的家具levelID
    local currentFurLevelCfg = furnitureLevelConfig[curFurLevelID] --当前选中的家具levelConfig
    local currentFurCfg = furnitureConfig[currentFurLevelCfg.furniture_id]
    --员工数据
...
function CycleToyBuildingUIView:RoomNeedUpgrade()
    local curFurLevelID = self.factoryFurData[tostring(selectIndex)].id --当前选中的家具levelID
    local currentFurLevelCfg = furnitureLevelConfig[curFurLevelID] --当前选中的家具levelConfig
    local currentFurCfg = furnitureConfig[currentFurLevelCfg.furniture_id]
    local nextFurLevelCfg = self.m_currentInstanceModel:GetFurlevelConfig(currentFurCfg.id, currentFurLevelCfg.level + 1)
...
function CycleToyBuildingUIView:OnFurUpgradeBtnClicked(showResNotEnoughTip)
    local curFurLevelID = self.factoryFurData[tostring(selectIndex)].id --当前选中的家具levelID
    local currentFurLevelCfg = furnitureLevelConfig[curFurLevelID] --当前选中的家具levelConfig
    local currentFurCfg = furnitureConfig[currentFurLevelCfg.furniture_id]
    local nextFurLevelCfg = self.m_currentInstanceModel:GetFurlevelConfig(currentFurCfg.id, currentFurLevelCfg.level + 1)
    ...
    GameTableDefine.CycleToyPopUI:PackTrigger(5, self.roomID)
    
    return true
end

function CycleToyBuildingUIView:ShowFactoryUI(roomID, select, isFirst)
...
function CycleToyBuildingUIView:ShowSupplyBuildingUI(roomID, select, isFirst)
    isOpen = isFirst
    if select then
        selectIndex = select
    else
...
function CycleToyBuildingUIView:UpdateSupplyBuildingFurItem(index,trans)
    index = index + 1
...
function CycleToyBuildingUIView:ShowWharfUI(roomID,select,isFirst)
    isOpen = isFirst
    if select then
        selectIndex = select
    else
...
function CycleToyBuildingUIView:UpdateWharfFurItem(index,trans)
    index = index + 1
    local go = trans.gameObject
...
function CycleToyBuildingUIView:GetFurnitureMaxlevel(furnitureID)
...
function CycleToyBuildingUIView:SelectFurniture(index,furData)
...
--region 英雄
---刷新英雄信息
function CycleToyBuildingUIView:RefreshHero()
    local roomCfg = roomsConfig[self.roomID]
    local heroID = roomCfg.hero_id
    local heroData = CycleToyHeroManager:GetHeroData(heroID)
    local heroLevel = heroData.level
    local heroConfig = CycleToyHeroManager:GetHeroConfig(heroID,heroLevel)
    ...
    if CycleToyHeroManager:IsMaxLevel(heroID) then
        ...
        local heroNextConfig = CycleToyHeroManager:GetHeroConfig(heroID,heroLevel+1)
        ...
        if not isEnough then
            GameTableDefine.CycleToyPopUI:PackTrigger(2)
        end
    end
end

---英雄升级按钮
function CycleToyBuildingUIView:OnHeroUpgradeBtnDown(showResNotEnoughTip)
    local roomCfg = roomsConfig[self.roomID]
    local heroID = roomCfg.hero_id
    local result,nextLevelCfg = CycleToyHeroManager:UpgradeHero(heroID)
    if result == CycleToyHeroManager.HeroUpgradeResult.Success then
        self:RefreshHero()
        self:RefreshFurnitureInfo()
        CycleToyMainViewUI:Refresh()
        return true
    elseif result == CycleToyHeroManager.HeroUpgradeResult.ResNotEnough then
        --if showResNotEnoughTip then
        --    ChooseUI:Choose("TXT_INSTANCE_SHOP_TIP",function()
        --        GameTableDefine.CycleToyShopUI:GetView()
        --    end)
        --end
    elseif result == CycleToyHeroManager.HeroUpgradeResult.LevelMax then
        --满级
    end
end
--endregion
...
function CycleToyBuildingUIView:OnUpdate()
...
function CycleToyBuildingUIView:InitUpgradePanel(isClickUpgradeBtn)
...
function CycleToyBuildingUIView:RefreshRoomUpgradePanelDynamic()
    ...
    elseif roomData.needShowUpgradeAnim then
        ...
        self:DestroyModeUIObject()
        CycleToyBuildingUI:DoFactoryUpgradeProcess(roomID,1)
...
function CycleToyBuildingUIView:GetRoomUpgradeNodeList(roomID)
    local nodeList = {1}
    local index = 2
...
---刷新房间升级界面里显示的所有信息
function CycleToyBuildingUIView:RefreshRoomUpgradePanel()
...
---按下房间升级界面中的升级按钮
function CycleToyBuildingUIView:OnRoomUpgradeBtnDown()
    local roomData = self.m_currentInstanceModel:GetCurRoomData(self.roomID)
    --升级中
...
---按下房间升级跳过按钮
function CycleToyBuildingUIView:OnUpgradeSkipBtnDown()
    local roomData = self.m_currentInstanceModel:GetCurRoomData(self.roomID)
...
---点击对话框
function CycleToyBuildingUIView:OnPersonTalkBtnDown()
    local roomID = self.roomID
    local roomCfg = roomsConfig[self.roomID]
    ...
    if not isSelling then
        ...
        self:DestroyModeUIObject(false,function()
            ...
            CycleInstanceDataManager:GetCurrentModel():GetScene():LookAtSceneGO(sellID,nil,nil,nil,function()
                GameTableDefine.CycleToySellUI:ShowWharfUI(sellID)
        ...
        if not BigNumber:CompareBig(storedResCount,needResCount) then
            ...
            self:DestroyModeUIObject(false,function()
                ...
                CycleInstanceDataManager:GetCurrentModel():GetScene():LookAtSceneGO(resRoomID,nil,nil,nil,function()
                    CycleToyBuildingUI:ShowFactoryUI(resRoomID)
                    GameTimer:CreateNewTimer(0.5,function()
                        GameUIManager:SetEnableTouch(true)
                    end)
                end)
...
---启动和更新家具闪烁
function CycleToyBuildingUIView:StartGlinting()
...
---关闭家具闪烁
function CycleToyBuildingUIView:StopGlinting()
    if not self.m_glintingTimer then
        return
    end
...
function CycleToyBuildingUIView:RefreshFurLevelReward(isUpgrade)
    ...
    if isUpgrade then
        if currentFurLevelCfg.milePoint_node then
            ...
            FlyIconsUI:ShowCycleMilepost(currentIndex,"Instance3_Milepost_Fly",BigNumber:FormatBigNumber(currentFurLevelCfg.milePoint_node),function()
                self.m_currentInstanceModel:AddScore(currentFurLevelCfg.milePoint_node)
                CycleToyMainViewUI:PlayMilestoneAnim()
            end)
        end
    end
...
--function CycleToyBuildingUIView:CheckRoomUpgradeGuide()
--    --1.是否是1号房间
