...
---@class CycleToyShopUIView:UIBaseView
local CycleToyShopUIView = Class("CycleToyShopUIView", UIView)

function CycleToyShopUIView:ctor()
    self.super:ctor()
    self.m_data = {}
    self.m_dataSize = {}
    self.typeToIndex = {}
...
function CycleToyShopUIView:OnEnter()
...
function CycleToyShopUIView:OnExit()
    --购买成功事件 反注册
...
function CycleToyShopUIView:InitList()
...
function CycleToyShopUIView:UpdateSaleData()
    local shopFrameCfg = {}
...
--对列表的每个item的单独刷新
function CycleToyShopUIView:UpdateListItem(i, tran)
    local index = i + 1
...
function CycleToyShopUIView:ShowList(data, refresh)
    self:CleanTimer()
    self.m_data = data
    if refresh then
        self.m_dataSize = {}
...
function CycleToyShopUIView:RefreshDiamond()
...
function CycleToyShopUIView:CleanTimer()
    
end

function CycleToyShopUIView:SetFrame1Details(go, data)
...
function CycleToyShopUIView:SetFrame3Details(go, data)
...
function CycleToyShopUIView:SetFrame2Details(go, data)
...
function CycleToyShopUIView:SetFrame4Details(go, data)
    local itemsName = {}
...
function CycleToyShopUIView:SetFramePesoDetails(go, data)
...
function CycleToyShopUIView:CheckDataAndGameObject(temp, data, index)
...
function CycleToyShopUIView:SetTempItemDetails(centGo, rootData, data)
    -- 特殊处理免费领取的代币商品
    if data.isFree then
        if self.freeGetTimer then
            GameTimer:StopTimer(self.freeGetTimer)
...
function CycleToyShopUIView:ShowItemDetails(rootData, data, valueShow)
...
function CycleToyShopUIView:CloseItemDetaild()
...
function CycleToyShopUIView:EnterToSpecial()
    if self.m_list then
...
--检测是否能购买对应等级的礼包
function CycleToyShopUIView:CheckCanGetLvlGift(shopID)
    local resultID = 0
...
function CycleToyShopUIView:TurnPage(index)
    if self.__timers["TurnPage"] then
        GameTimer:StopTimer(self.__timers["TurnPage"])
        self.__timers["TurnPage"] = nil
    end
...
---显示购买确认界面
function CycleToyShopUIView:ShowConfirmPanel(shopConfig,confirmCallback)
...
---隐藏购买确认界面
function CycleToyShopUIView:HideConfirmPanel()
    local frame1Panel = self:GetGoOrNil("frame1_info")
    if frame1Panel then
