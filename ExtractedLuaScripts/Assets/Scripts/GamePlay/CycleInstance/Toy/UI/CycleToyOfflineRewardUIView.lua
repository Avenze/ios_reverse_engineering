...
local CycleToyOfflineRewardUIView = Class("CycleToyOfflineRewardUIView", UIView)

function CycleToyOfflineRewardUIView:ctor()
    self.super:ctor()
end

function CycleToyOfflineRewardUIView:OnEnter()
    local disRewardData = {}
    ...
    self.rewardData = CycleToyModel:GetCurInstanceOfflineRewardData()
    ...
    if self.rewardData.rewardMoney and self.rewardData.rewardMoney > 0 then
        ...
        local tempData = { ["itemType"] = 31, ["icon"] = "icon_toy_cash2", ["str"] = BigNumber:FormatBigNumber(math.floor(self.rewardData.rewardMoney)) }
        table.insert(disRewardData, tempData)
    end
    
    if self.rewardData.rewardExp and self.rewardData.rewardExp > 0 then
        local iconName = CycleInstanceDataManager:GetInstanceBind().cashIcon
        local tempData = { ["itemType"] = 26, ["icon"] = "icon_toy_formulaExp", ["str"] = BigNumber:FormatBigNumber(math.floor(self.rewardData.rewardExp)) }
        table.insert(disRewardData, tempData)
    end
    
    if self.rewardData.rewardPoint and self.rewardData.rewardPoint > 0 then
        local iconName = CycleInstanceDataManager:GetInstanceBind().cashIcon
        local tempData = { ["itemType"] = 201, ["icon"] = "icon_toy_milePoint", ["str"] = BigNumber:FormatBigNumber(math.floor(self.rewardData.rewardPoint)) }
        table.insert(disRewardData, tempData)
    end
    ...
    local maxTime = CycleToyModel:GetMaxOfflineTime()
    local curDisOfflineTime = CycleToyModel:GetCurOfflineDisplayTime()
...
function CycleToyOfflineRewardUIView:OnExit()
...
return CycleToyOfflineRewardUIView