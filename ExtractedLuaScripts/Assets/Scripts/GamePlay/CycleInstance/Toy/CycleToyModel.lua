...
---@class CycleToyModel:CycleInstanceModelBase
---@field super CycleInstanceModelBase
local CycleToyModel = Class("CycleToyModel",CycleInstanceModelBase)
...
local CycleToyHeroManager = GameTableDefine.CycleToyHeroManager
...
local childLimit = 0   ---小孩数量上限
local childPoor = nil

local SCENE_CLASS_PATH = "GamePlay.CycleInstance.Toy.CycleToyScene"
local OFFLINE_TIME = 120

function CycleToyModel:ctor()
    self:getSuper(CycleToyModel).ctor(self)

    self.m_sceneClassPath = SCENE_CLASS_PATH

    self.instance_id = nil   ---副本ID(类型ID, 不是实例ID)
    self.m_currentScene = nil ---@type CycleToyScene
...
function CycleToyModel:Init(saveData)
    self:getSuper(CycleToyModel).Init(self,saveData)

    GameTableDefine.CycleToyHeroManager:Reset()
    GameTableDefine.CycleToyTaskManager:Reset()
    GameTableDefine.CycleToyBluePrintManager:Reset()
...
function CycleToyModel:InitConfigData()
    self:getSuper(CycleToyModel).InitConfigData(self)
    self.config_cy_instance_skill = ConfigMgr.config_cy_instance_skill[self.instance_id]
end

function CycleToyModel:OnEnter()
    self:getSuper(CycleToyModel).OnEnter(self)
    --初始化顾客上限
    self:RefreshClientLimit()
    self:RefreshChildLimit()
    clientQueue = {}
    isOpenExitPop = false
...
function CycleToyModel:Update(dt)

    self:getSuper(CycleToyModel).Update(self,dt)
    ...
    if CycleInstanceDataManager:GetInstanceIsActive() then
        ...
        --local instanceMainViewIndex = GameUIManager:GetUIIndex(ENUM_GAME_UITYPE.CYCLE_TOY_MAIN_VIEW_UI, true)
        if not isOpenExitPop and not self.m_currentScene:IsPlayingTimeLine() and GameUIManager:UIIsOnTop(ENUM_GAME_UITYPE.CYCLE_TOY_MAIN_VIEW_UI,true) then
            local txt = GameTextLoader:ReadText("TXT_INSTANCE_TIP_END")
            isOpenExitPop = true
            GameTableDefine.ChooseUI:CommonChoose(txt, function()
                CycleToyMainViewUI:Exit()
            end, false, function()
                isOpenExitPop = false
            end)
            -- 如果正在开启引导则强制退出引导
...
function CycleToyModel:OnPause()
    GameTableDefine.CycleToyOfflineRewardUI:CloseView()
...
function CycleToyModel:OnResume()
    self:CalculateInstanceOfflineReward()
...
function CycleToyModel:GetIsFirstEnter()
...
function CycleToyModel:SetIsFirstEnter()
    if self.saveData then
...
---获取引导ID
function CycleToyModel:GetGuideID()
    if self.saveData then
...
function CycleToyModel:SetGuideID(value)
    if self.saveData then
...
---override 计算在线收益
function CycleToyModel:CalculateInstanceOnlineReward(dt)
    self:CalculateFactoryUnitOutput(dt)
    self:CreateClient(dt)
end

---计算工厂单位时间的产量
function CycleToyModel:CalculateFactoryUnitOutput(dt)
...
function CycleToyModel:RefreshClientLimit()
...
function CycleToyModel:RefreshChildLimit()
    local roomLevelSort = self:GetHighestUnlockedRoomByRoomLevel()
    if not self.m_currentScene then
        return
    end
    if #roomLevelSort == 0 then
        childLimit = 0
    ...
    if childLimit < roomLevelSort[1].kid_num then
        childLimit = roomLevelSort[1].kid_num
        childPoor = roomLevelSort[1].kid_id
        self:CreateChild()
    end
end
...
function CycleToyModel:GetHighestUnlockedRoomByRoomLevel()
    local roomLevelSort = {}
    for k,v in pairs(self.roomsData) do
...
---计算码头输出量
function CycleToyModel:CreateClient(dt)
    ...
    if not self:ProductIsSelling(25) then
        return
    end
    if clientComeTimer <= comeCD then
        ...
        --生成顾客
        if CycleToyAIBlackBoard:GetClientCount() >= clientLimit then
            return
        end
        ...
        printf("新顾客到来:  " .. id)
        CycleToyAIBlackBoard:AddClient()
        clientComeTimer = 0
    end
end

function CycleToyModel:CreateChild()
    local curChildList = ActorManager:GetActorByType("CycleToyChild")
    local curNum = curChildList and #curChildList or 0
    if curNum >= childLimit then
        return
    end
    if curNum >= CycleToyAIBlackBoard:GetChildPlayPosCount() then
        return
    end
    -- 生成气氛组
    local count = childLimit - curNum
    for i = 1, count do
        local id = childPoor[math.random(1, #childPoor)]
        self.m_currentScene:CreateChild(id)
        printf("创建新的气氛组:  " .. id)
    end
end

function CycleToyModel:SellAllProduct(productions, preview)
    ...
    for k, v in pairs(produtions) do
        ...
        local bluePrintMoneyBuff,bluePrintMileBuff = CycleToyBluePrintManager:GetProductBuffValue(tonumber(k))
...
---设置副本产品信息
function CycleToyModel:SetProdutionsData(productions)
...
---获取副本产品信息
function CycleToyModel:GetProdutionsData()
...
---override 增加当前副本金钱
function CycleToyModel:AddCurInstanceCoin(num,showFlyIcon)

    self:getSuper(CycleToyModel).AddCurInstanceCoin(self,num,showFlyIcon)
    self:AddTaskAccumulateCoin(num)

    CycleToyMainViewUI:Refresh()
    CycleToyBuildingUI:RefreshView()
end

---获取某种资源的单位产量和消耗 (/min)
function CycleToyModel:GetProductionAndConsumptionPerMin(resourcesID)
    --TODO
...
---获取当前出售物品每分钟售价 (/min)
function CycleToyModel:GetInComePerMinute()
    local priceSum = 0
    for k, v in pairs(self.roomsConfig) do
        if self:ProductIsSelling(v.production) then
            ...
            --蓝图BUFF
            local bluePrintMoneyBuff,bluePrintMileBuff = CycleToyBluePrintManager:GetProductBuffValue(v.production)
...
---获取当前卖出的资源
function CycleToyModel:GetCurSellingProduct()
    local selling = {}
...
---产品是否正在出售
function CycleToyModel:ProductIsSelling(resID)
    local portRoomData = self:GetRoomDataByType(4)
    for k, v in pairs(portRoomData[1].furList) do
        if v.isOpen then
            local furLevelCfg = self.furnitureLevelConfig[v.id]
            if furLevelCfg.resource_type == resID then
                return true
            end
        end
    end
...
function CycleToyModel:HaveProductCanSale()
    local portRoomData = self:GetRoomDataByType(4)
    for k, v in pairs(portRoomData[1].furList) do
...
---获取一个房间每分钟的收益
function CycleToyModel:GetRoomIncomePerMin(roomID)
    --TODO
...
---获取当前的最高收益
function CycleToyModel:GetCurHighestProfit(isSelling)
    --TODO
...
---获取某种资源的锁定状态
function CycleToyModel:GetResLockedState(resourcesID)
    --TODO
    for k,v in pairs(self.roomsData) do
        local roomID = v.roomID
        local roomCfg = self.roomsConfig[roomID]
...
---获取当前售卖货架的index
function CycleToyModel:GetSellingIndex()
    --先找港口房间的roomData
...
function CycleToyModel:GetSellingRoomID()
    --先找港口房间的roomData
    local portRoomData = nil
    for k, v in pairs(self.saveData.m_RoomData) do
        local roomCfg = self.roomsConfig[tonumber(v.roomID)]
...
---获取一个房间单位时间的产品产量
function CycleToyModel:GetRoomProduction(roomID)
    ...
    local heroBuff = CycleToyHeroManager:GetHeroCurrentBuff(self.roomsConfig[roomID].hero_id)
    ...
    --蓝图BUFF
    local roomCfg = self:GetRoomConfigByID(roomID)
    local bluePrintMoneyBuff,bluePrintMileBuff = CycleToyBluePrintManager:GetProductBuffValue(roomCfg.production)
...
---获取生产某种资源的房间
function CycleToyModel:GetRoomByProduction(productionID)
    for k,v in pairs(self.roomsConfig) do
...
---获取一个房间单位时间的材料消耗
function CycleToyModel:GetRoomMatCost(roomID)
    local count = 0
    local roomCfg = self.roomsConfig[roomID]
...
---获取房间员工数量
function CycleToyModel:GetWorkerNum(roomID)
...
---override 购买房间
function CycleToyModel:BuyRoom(roomID)
    --设置存档信息
    ...
    local buffID = self:GetSkillIDByType(CycleToyModel.SkillTypeEnum.ReduceUpgradeCosts)
    ...
    -- 礼包弹窗触发事件
    CycleToyPopUI:PackTrigger(CycleToyPopUI.EventType.buyRoom, roomID)
    ...
    if lastRoomData then
        ...
        local roomCfg = self:GetRoomConfigByID(roomID)
        local heroData = CycleToyHeroManager:GetHeroData(roomCfg.hero_id)
...
function CycleToyModel:CheckRoomCondition(roomID)
    local roomData = self:GetCurRoomData(roomID)
...
---override 播放房间升级动画
function CycleToyModel:DoRoomUpgradeAnim(roomID)
    if self:getSuper(CycleToyModel).DoRoomUpgradeAnim(self,roomID) then
        --TODO  生成家具，镜头表现等
    end
end

---override 升级房间完成
function CycleToyModel:UpgradeRoomComplete(roomID)
    self:getSuper(CycleToyModel).UpgradeRoomComplete(self, roomID)
    self:RefreshClientLimit()
    self:RefreshChildLimit()
end

---override 购买家具
function CycleToyModel:BuyFurniture(roomID, index, furLevelID)
    local roomData = self.roomsData[roomID]
    ...
    self:AddCurInstanceCoin(-cost)
    CycleToyMainViewUI:Refresh()
    GameTableDefine.CycleToyPopUI:PackTrigger(7, furLevelCfg.id)
    --增加升级埋点,每5级埋点一次
    ...
    if furLevelCfg.level == 1 and furLevelCfg.isPresonFurniture then
        CycleToyMainViewUI:CallNotify(1)
    end
end

---返回当前指定时间产出的内容
function CycleToyModel:GetCurProductionsByTime(timeSeconds)
    local result = {}
    --资源产出的相关计算
...
---获取下一个可出售的房间
---return roomCfg
function CycleToyModel:GetNextRoomCanSelling()
    local roomLevelSort = {}
    for k,v in pairs(self.roomsData) do
        local roomID = tonumber(k)
        local roomCfg = self:GetRoomConfigByID(roomID)
...
function CycleToyModel:GetScene()
    return self.m_currentScene
end

---场景是否显示标志性建筑
function CycleToyModel:ShowSpecialGameObject(flag)
    self.m_currentScene:ShowSpecialGameObject(flag)
end

---地标购买成功的动画播放
function CycleToyModel:BuySpecialGameObjectAnimationPlay()
...
---override 设置积分等级
function CycleToyModel:SetCurInstanceKSLevel(value)
    if self.saveData then
        self.saveData.m_CurKSLevel = value
        CycleToyMainViewUI:CallNotify(3)
        CycleToyMainViewUI:Refresh()
        --埋点
        ...
        local heroLevel = CycleToyHeroManager:GetHeroData(heroID).level
        local skillData = self:GetCurSkillData()

        local skill1Level = GameTableDefine.CycleToySkillUI:GetCurSkillItemCfg(skillData[1]).skill_level
        local skill2Level = GameTableDefine.CycleToySkillUI:GetCurSkillItemCfg(skillData[2]).skill_level
        local skill3Level = GameTableDefine.CycleToySkillUI:GetCurSkillItemCfg(skillData[3]).skill_level
...
---副本内特殊商品购买的回调
function CycleToyModel:InstanceShopBuySccess(shopID,showFlyIcon,showRewardUI,complex)
    ...
    if 27 == shopCfg.type then
        ...
        elseif showRewardUI then
            --获得奖励界面
            GameTableDefine.CycleToyRewardUI:ShowGetReward({shop_id = shopID,count = 1,param = skillInfos},true)
        end
        return skillInfos
    end
    ...
    if 36 == shopCfg.type then
        ...
        CycleToyBluePrintManager:ChangeUpgradeResCount(shopCfg.param[1],num)
...
---获取免费代币冷却时间(商店)
function CycleToyModel:GetSlotFreeCoinCDTime()
    local isFirst = false
...
---获取免费代币冷却时间(广告)
function CycleToyModel:GetSlotADFreeCoinCDTime()
    local isFirst = false
...
---获取当前免费币当日领取次数(商店)
function CycleToyModel:GetCurFreeSlotCoinTimes()
...
---获取当前免费币当日领取次数(广告)
function CycleToyModel:GetCurADFreeSlotCoinTimes()
...
---获取当前标志性建筑是否购买
function CycleToyModel:GetLandMarkCanPurchas()
    --local instanceBind = CycleInstanceDataManager:GetInstanceBind()
    --local result = false
    --local landmarkID = instanceBind.landmark_id
...
function CycleToyModel:SetLandMarkCanPurchas()
    if self.saveData then
        ...
        if self.m_currentScene and self.m_currentScene.floatUI and self.m_currentScene.floatUI.Landmark then
            if self.m_currentScene.floatUI.Landmark.view then
                self.m_currentScene.floatUI.Landmark.view:Invoke("ShowCycleToySpacialGOBuyTipPop", false)
            end
        end
    end
end
...
---结算离线奖励
function CycleToyModel:CalculateInstanceOfflineReward()
...
function CycleToyModel:GetCurInstanceOfflineRewardData()
    return self.m_CurOfflineRewardData
end

---领取离线奖励
function CycleToyModel:GetOfflineReward()
...
function CycleToyModel:GetCurOfflineDisplayTime()
    local time = 0
...
function CycleToyModel:GetMaxOfflineTime()
    return 7200
end

---计算离线奖励
function CycleToyModel:CalculateOfflineRewards(offlineDuration, onlyEarnings, changeSaveData)
...
---获取英雄存档
function CycleToyModel:GetHeroSaveData()
    if self.saveData then
...
---获取当前英雄升级所需资源
function CycleToyModel:GetCurHeroExpRes()
...
function CycleToyModel:GetCurHeroExpResShow()
    if self.saveData then
...
---修改英雄升级资源
function CycleToyModel:ChangeCurHeroExpRes(changeValue)
...
---获取经验产出效率(/min)
function CycleToyModel:GetExpOutputPerMin()
    local result = 0
    for k, v in pairs(self.roomsConfig) do
        if v.room_category == 1 then
...
---获取积分产出效率(/min)
function CycleToyModel:GetPointsOutputPerMin()
    local result = 0
    for k, v in pairs(self.roomsConfig) do
        if v.room_category == 1 then
            local roomData = self:GetRoomDataByID(k)
...
---增加[任务开启后]积累副本[英雄经验]
function CycleToyModel:AddTaskAccumulateHeroExp(num)
...
function CycleToyModel:GetTaskAccumulateHeroExp()
...
function CycleToyModel:ResetTaskAccumulateHeroExp()
    if self.saveData then
...
function CycleToyModel:AddTaskAccumulateSlotCoin(num)
    if num > 0 then
...
function CycleToyModel:GetTaskAccumulateSlotCoin()
...
function CycleToyModel:ResetTaskAccumulateSlotCoin()
    if self.saveData then
        self.saveData.m_taskAccumulateSlotCoin = 0
    end
end
...
---获取当前技能ID列表
function CycleToyModel:GetCurSkillData()
...
---获取当前的技能点数
function CycleToyModel:GetCurSkillPoints(skillType)
...
---升级技能
function CycleToyModel:UpdateSkill(oldSkillID)
    local result = false
    local oldSkillCfg = GameTableDefine.CycleToySkillUI:GetCurSkillItemCfg(oldSkillID)
    local nextSkillCfg = GameTableDefine.CycleToySkillUI:GetNextSkillItemCfg(oldSkillID)
...
function CycleToyModel:CheckSkillCanUpdate()
    ...
    for k, v in pairs(self.saveData.m_Skill) do
        -- local oldSkillCfg = GameTableDefine.CycleToySkillUI:GetCurSkillItemCfg(v)
        local nextSkillCfg = GameTableDefine.CycleToySkillUI:GetNextSkillItemCfg(v)
...
function CycleToyModel:GetSkillBufferValueBySkillID(skillid)
    local skillCfg = GameTableDefine.CycleToySkillUI:GetCurSkillItemCfg(skillid)
...
---获取技能ID
function CycleToyModel:GetSkillIDByType(type)
...
---增加技能点数
function CycleToyModel:AddSkillPoints(addValue)
    local tmpAddValue = {}
    local keyTable = {}
    local maxTypeList = {}
    for k, v in pairs(self.saveData.m_curSkillPoints) do
        ...
        local skillNextCfg = GameTableDefine.CycleToySkillUI:GetNextSkillItemCfg(skillID)
...
function CycleToyModel:GetBuySkillTimes(shopID)
...
function CycleToyModel:AddBuySkillTimes(shopID)
...
---获取一次免费代币(广告)
function CycleToyModel:GetOneTimeSlotADFreeCoin()
...
---返回拉霸机当前代币数量
function CycleToyModel:GetCurSlotCoin()
...
---返回拉霸机当前代币倍率
function CycleToyModel:GetCurSlotMagnification()
    if not self.saveData.m_slot_machine then
        self.saveData.m_slot_machine = {}
        self.saveData.m_slot_machine.curToken = 0
        self.saveData.m_slot_machine.magnification = 1 --初始化的倍率为1
...
---设置拉霸机当前代币倍率
function CycleToyModel:SetCurSlotMagnification(magnification)
    if not self.saveData.m_slot_machine then
        self.saveData.m_slot_machine = {}
        self.saveData.m_slot_machine.curToken = 0
        self.saveData.m_slot_machine.magnification = 1 --初始化的倍率为1
...
function CycleToyModel:ChangeSlotCoin(changeVale)
...
function CycleToyModel:GetSlotMachineLevel()
    if self.saveData then
...
function CycleToyModel:SetSlotMachineLevel(level)
    if self.saveData then
...
function CycleToyModel:PushSlotNumAdd()
    if self.saveData then
...
function CycleToyModel:GetSlotPushNum()
...
function CycleToyModel:PushSlotRotioNum(rotio)
    if self.saveData then
...
function CycleToyModel:GetSlotPushNumRotio()
...
function CycleToyModel:GetRoomLevelAndHeroLevel(roomID, heroID)
    local furLevel = 0
    local heroLevel = 0
...
---override 设置引导为已完成状态
function CycleToyModel:SetGuideCompleted(guideID)
    self:getSuper(CycleToyModel).SetGuideCompleted(self,guideID)
    CycleToyMainViewUI:RefreshUIEntryByGuideState()

    if guideID == CycleInstanceClassDefine.GuideDefine.Toy.UnlockMilestone then
        --解锁地标气泡
        if self:GetLandMarkCanPurchas() then
            local scene = self:GetScene()
            if scene then
                if scene.floatUI.Landmark and scene.floatUI.Landmark.view then
                    scene.floatUI.Landmark.view:Invoke("ShowCycleToySpacialGOBuyTipPop", true)
                end
            end
        end
    end
...
function CycleToyModel:GetRoomUpgradeCost(roomID,level)
...
-----临时开启引导相关功能
-----返回引导是否完成
--function CycleToyModel:IsGuideCompleted(guideID)
--    return true
--end

---override 离线推送
function CycleToyModel:SendNotification()
    --仅推送给进入副本的用户
...
---检查是否应该还原离线推送的提示次数
function CycleToyModel:CheckRevertNotifyTimes()
    if not self.saveData.m_notify then
        return
    end
    local now = GameTimeManager:GetTheoryTime()
...
---获取本副本的蓝图Manager
function CycleToyModel:GetBluePrintManager()
    return CycleToyBluePrintManager
end

--蓝图是否能升级或材料是否能合成
--function CycleToyModel:CheckBlueprintTip()
--    return CycleToyBluePrintManager:CanUpgradeAnyProduction() or CycleToyBluePrintManager:CanCombineAnyRes()
--end

---override 奖励金钱 每分钟数额(拉霸机，广告等)
function CycleToyModel:GetRewardInComePerMinute()
    return self:GetInComePerMinute()
end

return CycleToyModel