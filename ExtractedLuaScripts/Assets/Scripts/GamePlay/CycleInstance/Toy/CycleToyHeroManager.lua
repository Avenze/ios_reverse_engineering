...
CycleToyHeroManager.HeroUpgradeResult = {
    Success = 1,
    ResNotEnough = 2,
    LevelMax = 3,
...
function CycleToyHeroManager:ctor()
...
---副本重开时，重置数据
function CycleToyHeroManager:Reset()
    self.m_isInitialized = false
end

function CycleToyHeroManager:Init()
...
function CycleToyHeroManager:GetHeroConfig(heroID,level)
    self:Init()
...
---根据英雄ID 获取buff值
---@return ConfigCyInstanceHero
function CycleToyHeroManager:GetHeroCurrentBuff(heroID)
...
---返回对应英雄的存档数据
---@return HeroData
function CycleToyHeroManager:GetHeroData(heroID)
    self:Init()
...
function CycleToyHeroManager:UpgradeHero(heroID)
    self:Init()
    local heroData = self:GetHeroData(heroID)
    ...
    if not curLevelConfig then
        error("没找到英雄配置")
        return CycleToyHeroManager.HeroUpgradeResult.CannotFindHeroConfig,nil
    end
    --判断是否有下一级的配置
    if nextLevelConfig then
        ...
        if CycleInstanceDataManager:GetCurrentModel():ChangeCurHeroExpRes("-"..requireRes) then
            ...
            return CycleToyHeroManager.HeroUpgradeResult.Success,nextLevelConfig
        else
            return CycleToyHeroManager.HeroUpgradeResult.ResNotEnough,nil
        end
    end
    return CycleToyHeroManager.HeroUpgradeResult.LevelMax,nil
end

---判断是否满级
function CycleToyHeroManager:IsMaxLevel(heroID)
    self:Init()
    local heroData = self:GetHeroData(heroID)
...
---是否可以升级
---@return boolean
function CycleToyHeroManager:CanUpgrade(heroID)
    self:Init()
    local heroData = self:GetHeroData(heroID)
    local curLevelConfig = self:GetHeroConfig(heroID,heroData.level)
    local nextLevelConfig = self:GetHeroConfig(heroID,heroData.level+1)
...
function CycleToyHeroManager:GetHeroMaxLevel(id)
    return 300
end

return CycleToyHeroManager