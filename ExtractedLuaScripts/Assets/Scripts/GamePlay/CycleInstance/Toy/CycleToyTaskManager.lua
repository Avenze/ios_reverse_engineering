...
function CycleToyTaskManager:ctor()
...
---初始化某个副本的任务存档数据,清空
function CycleToyTaskManager:InitTaskData()
...
---副本重开时重置数据
function CycleToyTaskManager:Reset()
    self.m_taskData = nil
end

---获取某副本的全部任务Config
function CycleToyTaskManager:GetTaskConfigsByInstanceID(instanceID)
...
function CycleToyTaskManager:GetTaskConfigByTaskID(taskID)
    local instanceID = CycleInstanceDataManager:GetCurrentModel().instance_id
...
---@return CycleIslandTaskConfig
function CycleToyTaskManager:GetNextTaskConfig(instanceID, curTaskID,taskShowType)
...
function CycleToyTaskManager:GetTaskData()
...
function CycleToyTaskManager:GetTaskState(taskID,showFlyIcon)
    ...
    if isComplete and showFlyIcon and taskStateData.needShowTip then
        ...
        if not GameTableDefine.CycleToyTaskUI.m_view and isGuideCompleted then
...
---@return boolean,number,number 是否完成,当前进度,总需求
function CycleToyTaskManager:GetType1TaskState(taskID)
    local taskConfig = self:GetTaskConfigByTaskID(taskID)
    local isComplete,curValue,needValue = false,0,1
...
function CycleToyTaskManager:FakeSetTaskAsComplete(taskID)
    local taskConfig = self:GetTaskConfigByTaskID(taskID)
    local showType = taskConfig.taskShowType
    local taskData = self:GetTaskData()
...
function CycleToyTaskManager:CompleteTask(taskID)
...
---完成类型1的任务，并加分
function CycleToyTaskManager:CompleteType1Task(taskID)
...
function CycleToyTaskManager:GetTaskShowValues(taskID)
    local taskConfig = self:GetTaskConfigByTaskID(taskID)
    local desc =  GameTextLoader:ReadText(taskConfig.desc)
    local descStr,haveStr,needStr

    local isComplete,have,need = CycleToyTaskManager:GetTaskState(taskID)

    local taskType = taskConfig.task_type
    if taskType == CycleToyTaskManager.TaskType.BuildRoom then
        --是否修建建筑
    ...
    elseif taskType == CycleToyTaskManager.TaskType.BuildingLevel then
        --建筑等级
        if isComplete then
    ...
    elseif taskType == CycleToyTaskManager.TaskType.CashAmount then
        --累计赚钱
        if isComplete then
    ...
    elseif taskType == CycleToyTaskManager.TaskType.HeroExpAmount then
        --累计获取经验
        if isComplete then
    ...
    elseif taskType == CycleToyTaskManager.TaskType.UseCoin then
        --累计花费货币
        --if isComplete then
...
---是否有任何任务已经完成，待领取奖励
function CycleToyTaskManager:IsAnyTaskIsComplete()
    local taskData = self:GetTaskData()
    for i = 1, 3 do
...
function CycleToyTaskManager:UpdateTaskState(showCompleteTip)
    local taskData = self:GetTaskData()
...
---刷新类型1(建造)任务的状态
function CycleToyTaskManager:RefreshType1TaskState()
    local taskData = self:GetTaskData()
    ---未完成的最小任务ID
...
function CycleToyTaskManager:GetType1TaskData()
...
---2. 同种类任务按id从小到大展示
function CycleToyTaskManager:GetCompleteTaskIDToShowInSlotMachine()
    local taskData = self:GetTaskData()
    for i = 3, 2 ,-1 do
        local taskID = taskData.currentTasks[i].taskID
        local isComplete,_,_ = self:GetTaskState(taskID)
