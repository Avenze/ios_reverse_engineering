...
---@class CycleNightClubTaskManager
...
CycleNightClubTaskManager.TaskType = {
    BuildRoom = 1,
    BuildingLevel = 2,
    CashAmount = 3,
    HeroExpAmount = 4,
...
function CycleNightClubTaskManager:ctor()
...
---初始化某个副本的任务存档数据,清空
function CycleNightClubTaskManager:InitTaskData()
...
---副本重开时重置数据
function CycleNightClubTaskManager:Reset()
    self.m_taskData = nil
end

---获取某副本的全部任务Config
function CycleNightClubTaskManager:GetTaskConfigsByInstanceID(instanceID)
...
function CycleNightClubTaskManager:GetTaskConfigByTaskID(taskID)
    local instanceID = CycleInstanceDataManager:GetCurrentModel().instance_id
...
---@return CycleIslandTaskConfig
function CycleNightClubTaskManager:GetNextTaskConfig(instanceID, curTaskID,taskShowType)
...
function CycleNightClubTaskManager:GetTaskData()
...
function CycleNightClubTaskManager:GetTaskState(taskID,showFlyIcon)
    ...
    if isComplete and showFlyIcon and taskStateData.needShowTip then
        ...
        if not GameTableDefine.CycleNightClubTaskUI.m_view and isGuideCompleted then
...
---@return boolean,number,number 是否完成,当前进度,总需求
function CycleNightClubTaskManager:GetType1TaskState(taskID)
    local taskConfig = self:GetTaskConfigByTaskID(taskID)
    local isComplete,curValue,needValue = false,0,1
...
function CycleNightClubTaskManager:FakeSetTaskAsComplete(taskID)
    local taskConfig = self:GetTaskConfigByTaskID(taskID)
    local showType = taskConfig.taskShowType
    local taskData = self:GetTaskData()
...
function CycleNightClubTaskManager:CompleteTask(taskID)
...
---完成类型1的任务，并加分
function CycleNightClubTaskManager:CompleteType1Task(taskID)
...
function CycleNightClubTaskManager:GetTaskShowValues(taskID)
    local taskConfig = self:GetTaskConfigByTaskID(taskID)
    local desc =  GameTextLoader:ReadText(taskConfig.desc)
    local descStr,haveStr,needStr

    local isComplete,have,need = CycleNightClubTaskManager:GetTaskState(taskID)

    local taskType = taskConfig.task_type
    if taskType == CycleNightClubTaskManager.TaskType.BuildRoom then
        --是否修建建筑
    ...
    elseif taskType == CycleNightClubTaskManager.TaskType.BuildingLevel then
        --建筑等级
        if isComplete then
    ...
    elseif taskType == CycleNightClubTaskManager.TaskType.CashAmount then
        --累计赚钱
        if isComplete then
    ...
    elseif taskType == CycleNightClubTaskManager.TaskType.HeroExpAmount then
        --累计获取经验
        if isComplete then
    ...
    elseif taskType == CycleNightClubTaskManager.TaskType.UseCoin then
        --累计花费货币
        --if isComplete then
...
---是否有任何任务已经完成，待领取奖励
function CycleNightClubTaskManager:IsAnyTaskIsComplete()
    local taskData = self:GetTaskData()
    for i = 1, 3 do
...
function CycleNightClubTaskManager:UpdateTaskState(showCompleteTip)
    local taskData = self:GetTaskData()
...
---刷新类型1(建造)任务的状态
function CycleNightClubTaskManager:RefreshType1TaskState()
    local taskData = self:GetTaskData()
    ---未完成的最小任务ID
...
function CycleNightClubTaskManager:GetType1TaskData()
...
---2. 同种类任务按id从小到大展示
function CycleNightClubTaskManager:GetCompleteTaskIDToShowInSlotMachine()
    local taskData = self:GetTaskData()
    for i = 3, 2 ,-1 do
        local taskID = taskData.currentTasks[i].taskID
        local isComplete,_,_ = self:GetTaskState(taskID)
...
return CycleNightClubTaskManager