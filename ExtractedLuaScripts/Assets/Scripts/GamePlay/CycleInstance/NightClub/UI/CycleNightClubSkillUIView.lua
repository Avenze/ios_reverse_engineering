...
local CycleNightClubSkillUIView = Class("CycleNightClubSkillUIView", UIView)

function CycleNightClubSkillUIView:ctor()
    self.super:ctor()
    self.container = {}
end

function CycleNightClubSkillUIView:OnEnter()
...
function CycleNightClubSkillUIView:OnPause()

end

function CycleNightClubSkillUIView:OnResume()
    self:ShowSkillPanel(CycleNightClubModel:GetCurSkillData())
end

function CycleNightClubSkillUIView:OnExit()

end

function CycleNightClubSkillUIView:ShowSkillPanel(skillData)
    -- local tempIcon = "btn_premiumpack10"
    --TODO：根据数据封装的模块获取当前的技能点数
    local curSkills = CycleNightClubModel:GetCurSkillData()
    for i = 1, 3 do
        local panelGo = self:GetGoOrNil("RootPanel/"..i.."Panel")
        
        local curSkillData = skillData[i]
        local curSkillCfg = GameTableDefine.CycleNightClubSkillUI:GetCurSkillItemCfg(curSkills[i])
        local nextSkillCfg = GameTableDefine.CycleNightClubSkillUI:GetNextSkillItemCfg(curSkills[i])
        local isMax = false
        if not nextSkillCfg then
            isMax = true
        end
        if panelGo and curSkillCfg then
            local curSkillPoints = CycleNightClubModel:GetCurSkillPoints(curSkillCfg.skill_type)
            ...
            if upgradeBtn then
                ...
                self:SetButtonClickHandler(upgradeBtn, function()
                    ...
                    if isHaveEnoughPoint then
                        if CycleNightClubModel:UpdateSkill(curSkillCfg.id) then
                            self:ShowSkillPanel(CycleNightClubModel:GetCurSkillData())
                        end
                    else
                        ...
                        GameTableDefine.CycleNightClubPopUI:PackTrigger(3)
...
return CycleNightClubSkillUIView
