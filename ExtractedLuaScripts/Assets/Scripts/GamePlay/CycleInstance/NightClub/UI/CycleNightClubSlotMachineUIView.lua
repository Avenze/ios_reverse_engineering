...
---@class CycleNightClubSlotMachineUIView:UIBaseView
local CycleNightClubSlotMachineUIView = Class("CycleNightClubSlotMachineUIView",UIBaseView)
...
function CycleNightClubSlotMachineUIView:ctor()
...
function CycleNightClubSlotMachineUIView:OnEnter()
    self.m_currentModel = CycleInstanceDataManager:GetCurrentModel()
    --按钮
    self.m_playButton = self:GetComp("RootPanel/Machine/Body/PushBtn","Button")
    self.m_playButton2 = self:GetComp("RootPanel/Machine/Body/PushBtn_2","Button")
    ...
    self.m_haveCoinNumText = self:GetComp("RootPanel/Machine/Body/PushBtn/num/num_1","TMPLocalization")
    self.m_haveCoinNotEnoughNumText = self:GetComp("RootPanel/Machine/Body/PushBtn/num/num_3","TMPLocalization")
    self.m_needCoinNumText = self:GetComp("RootPanel/Machine/Body/PushBtn/num/num_2","TMPLocalization")
    --Animator
    self.m_machineAnimator = self:GetComp("RootPanel","Animator")

    --tip
    self.m_tipPanelGO = self:GetGo("TipPanel")
    self.m_showTipBtn = self:GetComp("RootPanel/Machine/tips","Button")
    ...
    self:SetListItemCountFunc(self.m_tipScrollList,function()
        return CycleNightClubSlotMachineUI:GetMaxLevel()
    end)
    self.m_tipsGO = {
    ...
    local maxLevel = CycleNightClubSlotMachineUI:GetMaxLevel()
    --for i = 1, maxLevel do
    ...
    self:SetButtonClickHandler(self.m_taskButton,function()
        CycleNightClubTaskUI:OpenView(self.m_currentModel.instance_id or 4)
    end)
    ...
    --倍率
    local toggleParentGO = self:GetGo("RootPanel/Machine/Body/Multiple_area")
    self.m_toggles = {}
    for i=1,3 do
    ...
    self.m_curver = self:GetComp("RootPanel/Machine/Body/Slots","Curver")

    --最大等级
    local starParent = self:GetGoOrNil("RootPanel/Machine/Body/level")
    if starParent then
        self.m_starGOList = {}
        for i = 1, maxLevel do
    ...
    --local starPrefab = self:GetGo("RootPanel/Machine/Body/level/temp")
    --self.m_starGOList = {}
    --if starPrefab then
    ...
    --升级按钮与需求
    self.m_upgradeRootGO = self:GetGo("RootPanel/Machine/Body/upgradeBtn")
    self.m_upgradeBtn = self:GetComp("RootPanel/Machine/Body/upgradeBtn","Button")
    self:SetButtonClickHandler(self.m_upgradeBtn,handler(self,self.OnUpgradeBtnDown))
    self.m_upgradeCostNumText = self:GetComp("RootPanel/Machine/Body/upgradeBtn/cash/cost/num","TMPLocalization")
    self.m_upgradeNowNumText = self:GetComp("RootPanel/Machine/Body/upgradeBtn/cash/now/num","TMPLocalization")
    self.m_upgradeConditionText = self:GetComp("RootPanel/Machine/Body/upgradeBtn/tip/now/num","TMPLocalization")
    self.m_upgradeConditionGO = self:GetGo("RootPanel/Machine/Body/upgradeBtn/tip")
    self.m_upgradeMaxGO = self:GetGo("RootPanel/Machine/Body/maxlvl")
    --self.m_curLevelLights = {}
    --for i = 1, maxLevel do
    --    self.m_curLevelLights[i] = self:GetGo("RootPanel/Machine/Body/level/levelIcon_" .. i)
    --end
    self.m_maskGO = self:GetGo("RootPanel/Machine/Body/Slots/shade")
    self.m_fourthPrecinct = self:GetGo("RootPanel/Machine/Body/Slots/4")
    ...
    -- 小猪
    self.m_piggyBankGO = self:GetGo("RootPanel/PiggyBtn")
    self.m_piggyBankRedPointGO = self:GetGo("RootPanel/PiggyBtn/icon")
    self.m_piggyBankSlider = self:GetComp("RootPanel/PiggyBtn/Slider", "Slider")
    self.m_piggyBankCurValueText = self:GetComp("RootPanel/PiggyBtn/Slider/txt/txt1", "TMPLocalization")
    self.m_piggyBankMaxValueText = self:GetComp("RootPanel/PiggyBtn/Slider/txt/txt3", "TMPLocalization")
    self:RefreshPiggyBank()
    self:SetButtonClickHandler(self:GetComp("RootPanel/PiggyBtn","Button"),function()
...
function CycleNightClubSlotMachineUIView:BuildSlotMachineUIItems()
    --组装
    local slotRoot = self:GetGo("RootPanel/Machine/Body/Slots")
    self.m_slotItemGroups = {}
    RewardListCount = CycleNightClubSlotMachineUI.RewardCount
    for slotIndex = 1, CycleNightClubSlotMachineUI.SlotCount do
        ...
        for itemIndex = 1, RewardListCount do
            ...
            --初始化拉霸机图标
            local iconName = CycleNightClubSlotMachineUI:GetSlotSpriteName(slotIndex,itemIndex)
        ...
        slotGroup.animator = self:GetComp("RootPanel/Machine/Body/slot_anim/"..slotIndex,"Animator")
        self.m_slotItemGroups[slotIndex] = slotGroup
    end
...
---刷新拉霸机等级
function CycleNightClubSlotMachineUIView:RefreshStar()
...
function CycleNightClubSlotMachineUIView:Init()
...
---给整个拉霸机设置随机起始值
function CycleNightClubSlotMachineUIView:SetRandomValue()
    for slotIndex = 1, CycleNightClubSlotMachineUI.SlotCount do
        local randomValue = math.random(1,CycleNightClubSlotMachineUI.RewardCount) * 1.0 / CycleNightClubSlotMachineUI.RewardCount
        self:SetSlotValue(slotIndex,randomValue,0)
    end
end

function CycleNightClubSlotMachineUIView:RefreshMoneyNum()
    self.m_moneyText.text = self.m_currentModel:GetCurInstanceCoinShow()
end

function CycleNightClubSlotMachineUIView:RefreshCoinNum()
    if not self.m_currentModel then
        return
    end
    
    self.m_slotCoinNumText.text = tostring(self.m_currentModel:GetCurSlotCoin())
    local magIndex = self.m_currentModel:GetCurSlotMagnification()
    local needCoin = NeedCoinBase * CycleNightClubSlotMachineUI.SlotMagnificationConfig[magIndex][2]
...
function CycleNightClubSlotMachineUIView:SetSlotValue(slotIndex,progress,slope)
...
---最后一个Slot是否停止或者说没在转动
function CycleNightClubSlotMachineUIView:IsStop()
    local finalGroup = self.m_slotItemGroups[CycleNightClubSlotMachineUI.SlotCount]
    return finalGroup.time>= finalGroup.totalTime
end

---是否不在运行中
function CycleNightClubSlotMachineUIView:IsNotRunning()
    return not self.m_isRunning
end

---开始转动
function CycleNightClubSlotMachineUIView:Run(rewardResult)
    self.m_isRunning = true
    self.m_blockGO:SetActive(true)

    --是否可能是大奖，可能是大奖时最后一个Slot要多转一会儿
    local mayBeBigReward = CycleNightClubSlotMachineUI.m_maybeBigReward
    --设定最终结果,准备计算状态所需的参数
    local preStopTime = 0
    for slotIndex = 1, CycleNightClubSlotMachineUI.SlotCount do
        local group = self.m_slotItemGroups[slotIndex]
        if slotIndex == CycleNightClubSlotMachineUI.SlotCount and mayBeBigReward then
    ...
    self.m_runningSoundID = SoundEngine:PlaySFX("Assets/Res/Audio/cy4_slot_gear.ogg",true)
        ...
        self.m_runningTimer = Timer:CreateNewMilliSecTimer(1,function()
            for slotIndex = 1, CycleNightClubSlotMachineUI.SlotCount do
                ...
                if slotGroup.isRunning then
                    ...
                    if mayBeBigReward and slotIndex == CycleNightClubSlotMachineUI.SlotCount then
                    ...
                    if slotGroup.time >= slotGroup.totalTime - StopSFXValues[CycleNightClubSlotMachineUI.SlotCount][slotIndex] then
                        --提前播放音效
                        if not slotGroup.playedSound then
                            SoundEngine:PlaySFX("Assets/Res/Audio/SLOT_"..(CycleNightClubSlotMachineUI.SlotAudioConf[slotIndex])..".mp3")
                            slotGroup.playedSound = true
                            if slotIndex == CycleNightClubSlotMachineUI.SlotCount then
...
---转盘停止，输出奖励
function CycleNightClubSlotMachineUIView:OnStop()
    ...
    CycleNightClubSlotMachineUI:GetRewards()
    self.m_rewardResult = nil
end

---改变倍率
function CycleNightClubSlotMachineUIView:OnToggleValueChange(index)
...
---按钮按下，要判断是否能转动
function CycleNightClubSlotMachineUIView:OnPlayButtonClick()
    ...
    local magIndex = self.m_currentModel:GetCurSlotMagnification()
    local needCoins = NeedCoinBase * CycleNightClubSlotMachineUI.SlotMagnificationConfig[magIndex][2]
    if haveCoins>= needCoins and self.m_currentModel:ChangeSlotCoin(-needCoins) then
        self.m_rewardResult = CycleNightClubSlotMachineUI:Push()
        self:Run(self.m_rewardResult)

        -- 排行榜解锁
        GameTableDefine.CycleNightClubRankManager:unlockRankBoard()
        
        -- 小猪逻辑
        self:ChangePiggyBank(needCoins)
...
function CycleNightClubSlotMachineUIView:RefreshTipInfos()
    self.m_tipScrollList:SetSnapIndex(self.m_showTipLevel - 1)

    self.m_TipLeftBtn.gameObject:SetActive(self.m_showTipLevel>1)
    self.m_TipRightBtn.gameObject:SetActive(self.m_showTipLevel<CycleNightClubSlotMachineUI:GetMaxLevel())
...
function CycleNightClubSlotMachineUIView:ShowTipPanel(level)
    self.m_tipPanelGO:SetActive(true)
    self.m_showTipLevel = level or self.m_currentModel:GetSlotMachineLevel()
    self:RefreshTipInfos()
end

function CycleNightClubSlotMachineUIView:HideTipPanel()
    self.m_tipPanelGO:SetActive(false)
end

function CycleNightClubSlotMachineUIView:OnTipLeftBtnDown()
...
function CycleNightClubSlotMachineUIView:OnTipRightBtnDown()
    if self.m_showTipLevel < CycleNightClubSlotMachineUI:GetMaxLevel() then
        self.m_showTipLevel = self.m_showTipLevel + 1
        self:RefreshTipInfos()
    end
end
...
--function CycleNightClubSlotMachineUIView:RefreshADPanel()
...
---刷新任务按钮红点
function CycleNightClubSlotMachineUIView:RefreshTask()
    self.m_taskRedPointGO:SetActive(CycleNightClubTaskManager:IsAnyTaskIsComplete())
end

---播放广告给代币
--function CycleNightClubSlotMachineUIView:PlayAD()
--
--    local curTimes = self.m_currentModel:GetCurADFreeSlotCoinTimes()
--    local cdTime = self.m_currentModel:GetSlotADFreeCoinCDTime()
--    local canGetTimes = ConfigMgr.config_global.cycle_instance_freeCoinFre
...
function CycleNightClubSlotMachineUIView:PlayFlyIcon(flyIconPath,isFull,showValue)

    if self.m_flyingIconGO and self.m_flyingIconGO.transform then
        self.m_flyingIconGO:SetActive(false)
    end
...
function CycleNightClubSlotMachineUIView:PlayFlyIconSkill(flyIconPath,isFull,skillInfos)
    ...
    for k,v in pairs(skillInfos) do
        if v>0 then
            ...
            self:SetSprite(iconImage,"UI_Common","icon_nightClub_skill_"..k)
...
function CycleNightClubSlotMachineUIView:OnExit()
    ...
    self:CheckPiggyBank()
    if self.upgrade_slot then -- 升到2级，触发礼包
        local currStar = self.m_currentModel:GetSlotMachineLevel()
        GameTableDefine.CycleNightClubPopUI:PackTrigger(8, currStar)
    end
...
---刷新拉霸机升级按钮和需求
function CycleNightClubSlotMachineUIView:RefreshUpgradeInfo()
    local curLevel = self.m_currentModel:GetSlotMachineLevel()
    self:SetText("RootPanel/Machine/Body/level/levelNum", curLevel)
    ...
    local canUpgrade, roomID, cost = CycleNightClubSlotMachineUI:GetUpgradeCondition()
    ...
    local tmp = self:GetComp("RootPanel/Machine/Body/upgradeBtn/cash/now/num", "TMPLocalization")
...
--- 升级拉霸机
function CycleNightClubSlotMachineUIView:OnUpgradeBtnDown()
    local canUpgrade,roomID,cost = CycleNightClubSlotMachineUI:GetUpgradeCondition()
    if canUpgrade then
        local slot_lv = self.m_currentModel:GetSlotMachineLevel()
        self.m_currentModel:AddCurInstanceCoin(-cost)
        self.m_currentModel:SetSlotMachineLevel(slot_lv + 1)
        self.upgrade_slot = true
        if slot_lv == 2 then
            ---设置蓝图初始资金, 夜店slot三级给
            GameTableDefine.CycleNightClubBluePrintManager:InitBPOrigin()
        end

        CycleNightClubSlotMachineUI:Init()
...
---显示拉霸机内部文字提示
function CycleNightClubSlotMachineUIView:ShowInnerTip(textID)
...
---刷新快速任务的内容,每秒刷新
function CycleNightClubSlotMachineUIView:UpdateFastTask()
    local completeTaskID = CycleNightClubTaskManager:GetCompleteTaskIDToShowInSlotMachine()
    if completeTaskID then
        if completeTaskID ~= self.m_taskPopupTaskID then
            ...
            local descStr,haveStr,needStr,isComplete = CycleNightClubTaskManager:GetTaskShowValues(completeTaskID)
...
---快速获取任务奖励
function CycleNightClubSlotMachineUIView:FastGetTaskReward()
    if self.m_taskPopupTaskID then
        local taskConfig = CycleNightClubTaskManager:GetTaskConfigByTaskID(self.m_taskPopupTaskID)
        local getReward = false
        if taskConfig.taskShowType == 1 then
            getReward = CycleNightClubTaskManager:CompleteType1Task(taskConfig.id)
        else
            getReward = CycleNightClubTaskManager:CompleteTask(taskConfig.id)
        end
        if getReward then
...
function CycleNightClubSlotMachineUIView:OnResume()
    self:RefreshCoinNum()
end

-- 拉霸机给小猪充能
function CycleNightClubSlotMachineUIView:ChangePiggyBank(needCoins)
    local curValue, maxValue, piggyBankInfo = GameTableDefine.CycleInstanceDataManager:GetCurrentModel():GetPiggyBankValue()
    -- 未解锁，或已满级
    if not piggyBankInfo.unlock_time or not curValue or not maxValue or curValue >= maxValue then
        return
    end

    if curValue + needCoins < 0 then
        GameTableDefine.CycleInstanceDataManager:GetCurrentModel():ChangeSlotPiggyBank(needCoins)
    elseif curValue + needCoins == 0 then -- 本轮之后，刚刚解锁，直接显示出来
        GameTableDefine.CycleInstanceDataManager:GetCurrentModel():ChangeSlotPiggyBank(needCoins)
        self:RefreshPiggyBank()
        GameTableDefine.CycleNightClubMainViewUI:RefreshPiggyBank()
    elseif curValue + needCoins > 0 then
        if curValue < 0 then -- 从负到正，先增加到0，显示出节点，再播动画，将剩下的充能
            GameTableDefine.CycleInstanceDataManager:GetCurrentModel():ChangeSlotPiggyBank(-curValue)
            self:RefreshPiggyBank()
            needCoins = needCoins + curValue
        end

        GameTableDefine.CycleInstanceDataManager:GetCurrentModel():ChangeSlotPiggyBank(needCoins)
        GameTableDefine.FlyIconsUI:SetCycleInstanceNum({
            { ["itemType"] = 302, ["icon"] = "", ["str"] = tostring(needCoins or "") }
        }, function()
            self:RefreshPiggyBank()
            GameTableDefine.CycleNightClubMainViewUI:RefreshPiggyBank()
        end)
...
-- 刷新小猪状态
function CycleNightClubSlotMachineUIView:RefreshPiggyBank()
...
--- 检查小猪是否满
function CycleNightClubSlotMachineUIView:CheckPiggyBank()
    local curValue, maxValue, piggyBankInfo = GameTableDefine.CycleInstanceDataManager:GetCurrentModel():GetPiggyBankValue()
    if not curValue or not maxValue or curValue < 0 then
        self.m_piggyBankGO:SetActive(false)
        return
...
--- 打开小猪界面
function CycleNightClubSlotMachineUIView:OpenPiggyBank()
    GameTableDefine.CycleNightClubPiggyBankUI:GetView()
    GameUIManager:SetUITop(ENUM_GAME_UITYPE.CYCLE_NIGHT_CLUB_PIGGY_BANK_UI, true)
    GameUIManager:SetUITop(ENUM_GAME_UITYPE.FLY_ICONS_UI, true)
end

return CycleNightClubSlotMachineUIView