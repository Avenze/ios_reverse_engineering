...
---@class CycleNightClubShopUIView:UIBaseView
local CycleNightClubShopUIView = Class("CycleNightClubShopUIView", UIView)

function CycleNightClubShopUIView:ctor()
    self.super:ctor()
    self.m_data = {}
    self.m_dataSize = {}
    self.typeToIndex = {}
...
function CycleNightClubShopUIView:OnEnter()
    ...
    self.curInstanceLvl = CycleInstanceDataManager:GetCurrentModel():GetCurInstanceKSLevel()
    ShopManager:refreshBuySuccess(function(shopId)
        ShopManager:Buy(shopId, false, function()
            ...
            --         CycleInstanceDataManager:GetCurrentModel():InstanceShopBuySccess(tonumber(childId))
            ...
            if cfgShop.type == 27 or cfgShop.type == 26 or cfgShop.type == 30 then
                CycleInstanceDataManager:GetCurrentModel():InstanceShopBuySccess(shopId,true)

                if cfgShop.type == 26 then
                    --刷新主界面的英雄经验显示
                    CycleNightClubMainViewUI:Refresh()
                end
            end
            if cfgShop.type == 23 then
    ...
    if not CycleInstanceDataManager:GetCurrentModel():GetLandMarkCanPurchas() then
...
function CycleNightClubShopUIView:OnExit()
    --购买成功事件 反注册
...
function CycleNightClubShopUIView:InitList()
...
function CycleNightClubShopUIView:UpdateSaleData()
    local shopFrameCfg = {}
   
    for i, v in pairs(CycleInstanceDataManager:GetCurrentModel().config_cy_instance_shop) do
    ...
    for i, v in pairs(shopFrameCfg or {}) do
        ...
        if isFrame1 and item.content then
            ...
            freeItemData.amount = CycleInstanceDataManager:GetCurrentModel().config_global.cycle_instance_freeCoinNum
            freeItemData.icon = CycleInstanceDataManager:GetCurrentModel().config_global.cycle_instance_freeCoinIcon
            freeItemData.leftTimes = CycleInstanceDataManager:GetCurrentModel().config_global.cycle_instance_freeCoinFre - CycleInstanceDataManager:GetCurrentModel():GetCurFreeSlotCoinTimes()
            freeItemData.adLeftTimes = CycleInstanceDataManager:GetCurrentModel().config_global.cycle_instance_freeCoinFre - CycleInstanceDataManager:GetCurrentModel():GetCurADFreeSlotCoinTimes()
            if freeItemData.leftTimes < 0 then
                freeItemData.leftTimes = 0
            end
            if freeItemData.adLeftTimes < 0 then
                freeItemData.adLeftTimes = 0
            end
            table.insert(item.content, 1, freeItemData)
        end
        if item ~= nil and Tools:GetTableSize(item) > 0 then
            if isFrame2 then
                if CycleInstanceDataManager:GetCurrentModel():GetLandMarkCanPurchas() then
...
--对列表的每个item的单独刷新
function CycleNightClubShopUIView:UpdateListItem(i, tran)
    local index = i + 1
...
function CycleNightClubShopUIView:ShowList(data, refresh)
    self:CleanTimer()
    self.m_data = data
    if refresh then
        self.m_dataSize = {}
...
function CycleNightClubShopUIView:RefreshDiamond()
...
function CycleNightClubShopUIView:CleanTimer()
    
end

function CycleNightClubShopUIView:SetFrame1Details(go, data)
    ...
    for k = 1, Tools:GetTableSize(data.content) do
        ...
        if v.isFree and v.leftTimes <= 0 and v.adLeftTimes <= 0 then
            local flag = 1
        else
...
function CycleNightClubShopUIView:SetFrame3Details(go, data)
...
function CycleNightClubShopUIView:SetFrame2Details(go, data)
    ...
    --添加当前的副本地标购买检测
    go:SetActive(CycleInstanceDataManager:GetCurrentModel():GetLandMarkCanPurchas())
    local trackPrice = 0
    if shopCfg.iap_id then
        ...
        self:SetText(go, "sale/moaiStatue/content/mBtn/text", price)
    end
    go:SetActive(true)
    local btn = self:GetComp(go, "sale/moaiStatue/content/mBtn", "Button")
    btn.interactable = true
    self:SetButtonClickHandler(btn, function()
        btn.interactable = false
        GameSDKs:TrackForeign("store", {source = 3, operation_type = 1, product_id = IAP:GetPurchaseId(shopCfg.iap_id), pay_type = 1, cost_num = trackPrice})
        Shop:CreateShopItemOrder(shopCfg.id, nil, 2)
    end)

end

function CycleNightClubShopUIView:SetFrame4Details(go, data)
    local itemsName = {}
                ...
                if shopCfg then
                    ...
                    self:SetButtonClickHandler(self:GetComp(tmpGo, "content/mBtn", "Button"), function()
                        ...
                        if data.info and data.info > 0 then
                            ...
                            GameSDKs:TrackForeign("store", {source = 3, operation_type = 1, product_id = IAP:GetPurchaseId(shopCfg.iap_id), pay_type = 1, cost_num = trackPrice})
                            Shop:CreateShopItemOrder(v.shopID, nil, 2)
                        end
                    end)
                    break;
                end
...
function CycleNightClubShopUIView:SetFramePesoDetails(go, data)
    print("Set Instance shop frame5")
    local temp = self:GetGo(go, "sale/temp")

    self:GetGo(go,"offvalue"):SetActive(CycleInstanceDataManager:GetCurrentModel():IsLastOneDay())
    self:SetText(go, "offvalue/num", string.format("-%d%%", CycleInstanceDataManager:GetCurrentModel():GetLastOneDayDiscount() * 100))
    ...
    for k = 1, Tools:GetTableSize(data.content) do
        ...
        local resReward, money = CycleInstanceDataManager:GetCurrentModel():CalculateOfflineRewards(time,false,false)
        ...
        local isInstanceLastOneDay = CycleInstanceDataManager:GetCurrentModel():IsLastOneDay()
        if (shopV.type == 22 or shopV.type == 25) and isInstanceLastOneDay then
            ...
            self:SetText(buyButton.gameObject, "price_new/text", Tools:SeparateNumberWithComma(math.ceil(shopV.diamond * (1-CycleInstanceDataManager:GetCurrentModel():GetLastOneDayDiscount()))))
        end
            ...
            function()
                if canBuy then
                    ShopManager:Buy(shopV.id, false, function()
                        ...
                        self:RefreshDiamond()
                        CycleInstanceDataManager:GetCurrentModel():AddCurInstanceCoin(value,true)
                        self:ShowList(self:UpdateSaleData(), true)
...
function CycleNightClubShopUIView:CheckDataAndGameObject(temp, data, index)
...
function CycleNightClubShopUIView:SetTempItemDetails(centGo, rootData, data)
    -- 特殊处理免费领取的代币商品
    if data.isFree then
        if self.freeGetTimer then
            GameTimer:StopTimer(self.freeGetTimer)
        ...
        self:SetButtonClickHandler(fBtn, function()
            ...
            if CycleInstanceDataManager:GetCurrentModel():GetOneTimeSlotFreeCoin(true) then
                self:ShowList(self:UpdateSaleData(), true)
            else
                fBtn.interactable = true
            end
        end)
        local aBtn = self:GetComp(centGo, "aBtn", "Button")
        self:SetButtonClickHandler(aBtn, function()
            aBtn.interactable = false
            local curTimes = CycleInstanceDataManager:GetCurrentModel():GetCurADFreeSlotCoinTimes()
            local cdTime = CycleInstanceDataManager:GetCurrentModel():GetSlotADFreeCoinCDTime()
            ...
            GameSDKs:PlayRewardAd(function()
                    CycleInstanceDataManager:GetCurrentModel():GetOneTimeSlotADFreeCoin()
        ...
        self.freeGetTimer = GameTimer:CreateNewTimer(1, function()
            --2024-11-26fy添加容错
            if not centGo or centGo:IsNull() then
                GameTimer:StopTimer(self.freeGetTimer)
                self.freeGetTimer = nil
                return
            end
            ...
            local aBtn = self:GetComp(centGo, "aBtn", "Button")
            local timeGO = self:GetGo(centGo, "timeRes/time")
            local textGO = self:GetGo(centGo, "timeRes/text")
            local numGo = self:GetGo(centGo, "timeRes/num")
            mBtn.gameObject:SetActive(false)
            dBtn.gameObject:SetActive(false)

            local freeCoinCD = CycleInstanceDataManager:GetCurrentModel():GetSlotFreeCoinCDTime()
            local canGetFreeTime = data.leftTimes 
            local adFreeCoinCD = CycleInstanceDataManager:GetCurrentModel():GetSlotADFreeCoinCDTime()
            local canGetADFreeTime = data.adLeftTimes 

            if canGetFreeTime > 0 then  -- 有免费领取次数
                if freeCoinCD > 0 then -- 免费领取在CD中
                    if canGetADFreeTime then    -- 有广告领取次数
                        if adFreeCoinCD > 0 then    -- 广告领取在CD中, 优先显示免费CD
                            local timeStr = GameTimeManager:FormatTimeLength(freeCoinCD)
                            ...
                            fBtn.interactable = false
                        else    -- 广告领取可用, 显示广告领取按钮
                            --local timeStr = GameTimeManager:FormatTimeLength(adFreeCoinCD)
                            timeGO:SetActive(false)
                            textGO:SetActive(true)
                            numGo:SetActive(true)
                            self:SetText(numGo, "", canGetADFreeTime)
                            fBtn.gameObject:SetActive(false)
                            aBtn.gameObject:SetActive(true)
                            aBtn.interactable = true
                        end
                    else    -- 无广告领取次数, 直接显示免费领取CD
                        local timeStr = GameTimeManager:FormatTimeLength(freeCoinCD)
                        timeGO:SetActive(true)
                        textGO:SetActive(false)
                        numGo:SetActive(false)
                ...
                else    -- 免费领取可用, 直接显示免费领取
                    timeGO:SetActive(false)
                    textGO:SetActive(true)
                    numGo:SetActive(true)
                    self:SetText(numGo, "", canGetFreeTime)
                    fBtn.gameObject:SetActive(true)
                    aBtn.gameObject:SetActive(false)
                    fBtn.interactable = true
                end
                ...
                if canGetADFreeTime then    -- 有广告领取次数
                    if adFreeCoinCD > 0 then    -- 广告领取在CD中, 显示广告CD
                        local timeStr = GameTimeManager:FormatTimeLength(adFreeCoinCD)
                        timeGO:SetActive(true)
                        textGO:SetActive(false)
                        numGo:SetActive(false)
    ...
    if timeResGo then
        ...
        local leftNum = data.limit - CycleInstanceDataManager:GetCurrentModel():GetBuySkillTimes(data.shopID)
        if leftNum < 0 then
            leftNum = 0
        end
    ...
    if (v.type == 22 or v.type == 25) then
        self:GetGo(centGo,"offvalue"):SetActive(CycleInstanceDataManager:GetCurrentModel():IsLastOneDay())
        self:SetText(centGo, "offvalue/num", string.format("-%d%%", CycleInstanceDataManager:GetCurrentModel():GetLastOneDayDiscount() * 100))
    end
    ...
    if data.limit > 0 then
        btn.interactable = (data.limit - CycleInstanceDataManager:GetCurrentModel():GetBuySkillTimes(data.shopID)) > 0
    end
    if v.iap_id then
        ...
        self:SetButtonClickHandler(btn, function()
            ...
            if rootData.info and rootData.info > 0 then
                ...
                GameSDKs:TrackForeign("store", {source = 3, operation_type = 1, product_id = IAP:GetPurchaseId(v.iap_id), pay_type = 1, cost_num = trackPrice})
                Shop:CreateShopItemOrder(data.shopID, nil, 2)
            end
        end)
    elseif v.diamond then
        ...
        if calTime > 0 then
            local itemInfo = GameTableDefine.CycleInstanceDataManager:GetCurrentModel():GetCurProductionsByTime(calTime)
            ...
            for i, v in ipairs(itemInfo or {}) do
                ...
                local resCfg = CycleInstanceDataManager:GetCurrentModel().resourceConfig[v.resourcesID]
                if resCfg then
                    if i == 1 then
        ...
        local function DiamondClick(selectData,_rootData)
            ...
            if type == 27 and data.limit - CycleInstanceDataManager:GetCurrentModel():GetBuySkillTimes(data.shopID) <= 0 then
                return
            end
                ...
                if canBuy then
                    ...
                    ShopManager:Buy(selectData.id, false, function()
                        ...
                        if data then
                            ...
                            if cfgShop.type == 27 or cfgShop.type == 26 or cfgShop.type == 30 then
                                CycleInstanceDataManager:GetCurrentModel():InstanceShopBuySccess(selectData.id,false,true)
                            end
                            self:ShowList(self:UpdateSaleData(), true)
                            self:RefreshDiamond()
                        end
        ...
        if (v.type == 22 or v.type == 25) and isInstanceLastOneDay then
            self:SetText(btn.gameObject, "text", Tools:SeparateNumberWithComma(math.ceil(v.diamond * CycleInstanceDataManager:GetCurrentModel():GetLastOneDayDiscount())))
            ...
            self:SetText(btn.gameObject, "price_new/text", Tools:SeparateNumberWithComma(math.ceil(v.diamond * (1-CycleInstanceDataManager:GetCurrentModel():GetLastOneDayDiscount()))))
        end

        self:GetGo(centGo, "propDesc/descTimeText"):SetActive(v.type == 26)
        --判断一下当前是不是技能购买，如果是的话是有购买限制的
        if data.limit > 0 and type == 27 and data.limit - CycleInstanceDataManager:GetCurrentModel():GetBuySkillTimes(data.shopID) <= 0 then
            canBuy = false
            btn.interactable = false
            if iconBtn then
...
function CycleNightClubShopUIView:ShowItemDetails(rootData, data, valueShow)
    ...
    self:SetButtonClickHandler(btn, function()
        btn.interactable = false
        GameSDKs:TrackForeign("store", {source = 3, operation_type = 1, product_id = data.id, pay_type = 3, cost_num = data.diamond})
        self:CloseItemDetaild()
        if canBuy then
            ...
            GameSDKs:TrackForeign("store", {source = 3, operation_type = 3, product_id = IAP:GetPurchaseId(data.diamond), pay_type = 3, cost_num = data.diamond})
            self:CloseItemDetaild()
            btn.interactable = true
        end
    end)
...
function CycleNightClubShopUIView:CloseItemDetaild()
...
function CycleNightClubShopUIView:EnterToSpecial()
    if self.m_list then
...
--检测是否能购买对应等级的礼包
function CycleNightClubShopUIView:CheckCanGetLvlGift(shopID)
    local resultID = 0
    local instanceBind = CycleInstanceDataManager:GetInstanceBind()
    for k, v in ipairs(CycleInstanceDataManager:GetCurrentModel().config_achievement_instance) do
...
function CycleNightClubShopUIView:TurnPage(index)
    if self.__timers["TurnPage"] then
        GameTimer:StopTimer(self.__timers["TurnPage"])
        self.__timers["TurnPage"] = nil
    end
...
---显示购买确认界面
function CycleNightClubShopUIView:ShowConfirmPanel(shopConfig,confirmCallback)
    ...
    if frame1Panel then
        ...
        if shopConfig.type == 26 then
            displayNum = BigNumber:FormatBigNumber(shopConfig.amount * CycleInstanceDataManager:GetCurrentModel():GetExpOutputPerMin())
        end
...
---隐藏购买确认界面
function CycleNightClubShopUIView:HideConfirmPanel()
    local frame1Panel = self:GetGoOrNil("frame1_info")
    if frame1Panel then
...
return CycleNightClubShopUIView