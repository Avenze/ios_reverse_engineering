...
function CycleNightClubAdUI:GetView()
    CycleNightClubModel = GameTableDefine.CycleInstanceDataManager:GetCurrentModel()
    self:GetUIModel()
    self.m_view = GameUIManager:SafeOpenUI(ENUM_GAME_UITYPE.CYCLE_NIGHT_CLUB_AD_UI, self.m_view, require("GamePlay.CycleInstance.NightClub.UI.CycleNightClubAdUIView"), self, self.CloseView)
    return self.m_view
end

function CycleNightClubAdUI:CloseView()
    GameUIManager:CloseUI(ENUM_GAME_UITYPE.CYCLE_NIGHT_CLUB_AD_UI)
    self.m_view = nil
    collectgarbage("collect")
end


function CycleNightClubAdUI:GetUIModel()
...
function CycleNightClubAdUI:RefreshUIModel()
    self:GetUIModel()
end

function CycleNightClubAdUI:ClaimResource(cb, onSuccess, onFail,id)
    ...
    local callback = function()
        ...
        CycleNightClubModel:AddCurInstanceCoin(self.m_model.money)

        GameTableDefine.FlyIconsUI:SetCycleInstanceNum({
            { ["itemType"] = 31, ["icon"] = "item_instance1_cash", ["str"] = BigNumber:FormatBigNumber(math.floor(self.m_model.money)) }
        })
...
function CycleNightClubAdUI:ShowPanel()
    self:GetView()
end