...
local CycleNightClubOfflineRewardUIView = Class("CycleNightClubOfflineRewardUIView", UIView)

function CycleNightClubOfflineRewardUIView:ctor()
    self.super:ctor()
end

function CycleNightClubOfflineRewardUIView:OnEnter()
    local disRewardData = {}
    ...
    self.rewardData = CycleNightClubModel:GetCurInstanceOfflineRewardData()
    ...
    if self.rewardData.rewardMoney and self.rewardData.rewardMoney > 0 then
        ...
        local tempData = { ["itemType"] = 31, ["icon"] = "icon_NtClub_cash2", ["str"] = BigNumber:FormatBigNumber(math.floor(self.rewardData.rewardMoney)) }
        table.insert(disRewardData, tempData)
    end
    
    if self.rewardData.rewardExp and self.rewardData.rewardExp > 0 then
        local iconName = CycleInstanceDataManager:GetInstanceBind().cashIcon
        local tempData = { ["itemType"] = 26, ["icon"] = "icon_NtClub_formulaExp", ["str"] = BigNumber:FormatBigNumber(math.floor(self.rewardData.rewardExp)) }
        table.insert(disRewardData, tempData)
    end
    
    if self.rewardData.rewardPoint and self.rewardData.rewardPoint > 0 then
        local iconName = CycleInstanceDataManager:GetInstanceBind().cashIcon
        local tempData = { ["itemType"] = 201, ["icon"] = "icon_NtClub_milePoint", ["str"] = BigNumber:FormatBigNumber(math.floor(self.rewardData.rewardPoint)) }
        table.insert(disRewardData, tempData)
    end
    ...
    local maxTime = CycleNightClubModel:GetMaxOfflineTime()
    local curDisOfflineTime = CycleNightClubModel:GetCurOfflineDisplayTime()
...
function CycleNightClubOfflineRewardUIView:OnExit()
...
return CycleNightClubOfflineRewardUIView