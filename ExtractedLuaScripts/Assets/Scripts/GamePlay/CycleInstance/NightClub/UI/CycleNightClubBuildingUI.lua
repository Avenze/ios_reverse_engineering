...
---@class CycleNightClubBuildingUI
...
function CycleNightClubBuildingUI:GetView()
    --print("打开副本建筑UI \n"..debug.traceback())
    self.m_view = GameUIManager:SafeOpenUI(ENUM_GAME_UITYPE.CYCLE_NIGHT_CLUB_BUILDING_UI, self.m_view, require("GamePlay.CycleInstance.NightClub.UI.CycleNightClubBuildingUIView"), self, self.CloseView)
    return self.m_view
end

function CycleNightClubBuildingUI:CloseView()
    --停止播放BGM
    SoundEngine:StopSFX(currentBGM)

    GameUIManager:CloseUI(ENUM_GAME_UITYPE.CYCLE_NIGHT_CLUB_BUILDING_UI)
    self.m_view = nil
    collectgarbage("collect")
end

---房间升级流程
function CycleNightClubBuildingUI:DoFactoryUpgradeProcess(roomID,furIndex)
    ...
    --1.隐藏主界面
    GameTableDefine.CycleNightClubMainViewUI:GuideTimeUIState(false)
    ...
    if decorationFurID then
        ...
        if decorationGO then
            GameTimer:CreateNewTimer(0.5,function()
                --2.在0.5s后,视角锁定到装饰
                local cameraFocus = GameTableDefine.CycleNightClubMainViewUI.m_view:GetComp("RoomLevel_SceneRange","CameraFocus")
                currentScene:LookAtGameObject(room.roomGO.roomBox,cameraFocus,false,function()
                    ...
                    GameTimer:CreateNewTimer(length,function()
                        ...
                        GameTimer:CreateNewTimer(1.2,function()
                            ...
                            GameTableDefine.CycleNightClubMainViewUI:GuideTimeUIState(true)
...
function CycleNightClubBuildingUI:ShowFactoryUI(roomID, furIndex)
...
function CycleNightClubBuildingUI:RefreshView()
    if self.m_view then
...
--显示补给建筑UI
function CycleNightClubBuildingUI:ShowSupplyBuildingUI(roomID, furIndex)
...
--显示码头UI
function CycleNightClubBuildingUI:ShowWharfUI(roomID, furIndex)
