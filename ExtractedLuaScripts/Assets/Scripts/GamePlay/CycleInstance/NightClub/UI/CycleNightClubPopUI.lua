...
function CycleNightClubPopUI:GetView()
    self.m_view = GameUIManager:SafeOpenUI(ENUM_GAME_UITYPE.CYCLE_NIGHT_CLUB_POP_UI, self.m_view, require("GamePlay.CycleInstance.NightClub.UI.CycleNightClubPopUIView"), self, self.CloseView)
    return self.m_view
end

function CycleNightClubPopUI:CloseView()
    GameUIManager:CloseUI(ENUM_GAME_UITYPE.CYCLE_NIGHT_CLUB_POP_UI)
    self.m_view = nil
    collectgarbage("collect")
end

function CycleNightClubPopUI:OpenView(initID)
...
---6-首次升级蓝图并退出卖场UI时触发
---7-生产设施等级达标时触发
function CycleNightClubPopUI:PackTrigger(eventType, param)
...
function CycleNightClubPopUI:AddPackData(giftsCfg)
    local refresh = false
...
function CycleNightClubPopUI:StartTimer()
    if not self.Timer then
        self.Timer = GameTimer:CreateNewTimer(1, function()
            if GameUIManager:GetEnableTouch() and GameUIManager:UIIsOnTop(ENUM_GAME_UITYPE.CYCLE_NIGHT_CLUB_MAIN_VIEW_UI, true, IgnoreList) then
                local isCompletedGuide = CycleInstanceDataManager:GetCurrentModel():IsGuideCompleted(CycleInstanceClassDefine.GuideDefine.NightClub.UnlockMilestone)
