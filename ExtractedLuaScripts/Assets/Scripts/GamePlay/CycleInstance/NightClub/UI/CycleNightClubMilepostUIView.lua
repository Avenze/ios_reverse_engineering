...
local CycleNightClubMilepostUIView = Class("CycleNightClubMilepostUIView", UIView)
local CycleInstanceClassDefine = require("GamePlay.CycleInstance.CycleInstanceDefine")

local UnityHelper = CS.Common.Utils.UnityHelper

function CycleNightClubMilepostUIView:ctor()
    self.super:ctor()
    self.m_data = {}
    self.m_notClaimGO = nil ---当前未获取奖励的ListItemGO, 用来给引导用
end

function CycleNightClubMilepostUIView:OnEnter()
...
function CycleNightClubMilepostUIView:OnPause()

end

function CycleNightClubMilepostUIView:OnResume()
end

function CycleNightClubMilepostUIView:OnExit()
    if self.tiemr then
        GameTimer:_RemoveTimer(self.tiemr)
    end
    local currentModel = CycleInstanceDataManager:GetCurrentModel()
    --关闭界面时判断引导
    if not currentModel:IsGuideCompleted(CycleInstanceClassDefine.GuideDefine.NightClub.UnlockSlotMachine) then
        --开启引导
        GameTableDefine.GuideManager.currStep = CycleInstanceClassDefine.GuideDefine.NightClub.UnlockSlotMachine
        GameTableDefine.GuideManager:ConditionToStart()
        currentModel:SetGuideCompleted(CycleInstanceClassDefine.GuideDefine.NightClub.UnlockSlotMachine)
    end
end

function CycleNightClubMilepostUIView:ShowMilepostPanel(data, curData, index,scrollToNewest)
    -- local cur = {}
    --     --TODO:待实装内容
...
function CycleNightClubMilepostUIView:UpdateMilepostRewardList(index, tran)
        ...
        self:SetButtonClickHandler(curBtn, function()
            if data.rewardStatus == 3 then
                ...
                if rewards and next(rewards) ~= nil then
                    ...
                    if getFlag then
                        if dispRewards then
                            GameTableDefine.CycleNightClubRewardUI:ShowGetReward(dispRewards)
                        else
                            GameTableDefine.CycleNightClubRewardUI:ShowGetReward(rewards)
                        end
...
function CycleNightClubMilepostUIView:OpenRewardInfo(curGo, rewardInfoData)
...
function CycleNightClubMilepostUIView:GetNotClaimedRewardBtn()
...
function CycleNightClubMilepostUIView:GetComp(obj, child, uiType)
    if obj == "GetNotClaimedRewardBtn" then
        return self:GetNotClaimedRewardBtn()
    else
        return self:getSuper(CycleNightClubMilepostUIView).GetComp(self,obj, child,uiType)
    end
end

return CycleNightClubMilepostUIView