...
---@class CycleNightClubTaskUIView:UIBaseView
local CycleNightClubTaskUIView = Class("CycleNightClubTaskUIView", UIView)
...
function CycleNightClubTaskUIView:ctor()
...
function CycleNightClubTaskUIView:OnEnter()
    ...
    for i = 1, TASK_ITEM_COUNT do
        ...
        self:SetButtonClickHandler(taskUIItem.jumpBtn,function()
            if CycleInstanceDataManager:GetInstanceState(CycleInstanceDataManager.instance_id) ~= CycleInstanceDataManager.instanceState.isActive then
                GameTableDefine.CycleNightClubSlotMachineUI:CloseView()
...
function CycleNightClubTaskUIView:OnExitBtnDown()
    self:DestroyModeUIObject()
end

function CycleNightClubTaskUIView:OnExit()
...
function CycleNightClubTaskUIView:RewardBtnDown(taskUIItem)
    ...
    if getReward then
        ...
        if taskConfig.taskShowType == 1 then
            self:RefreshTaskInfo(taskUIItem,CycleNightClubTaskManager:GetType1TaskData())
        else
...
-----@param taskUIItem TaskUIItem
function CycleNightClubTaskUIView:JumpBtnDown(taskUIItem)
...
function CycleNightClubTaskUIView:RefreshTaskInfo(taskUIItem,taskData)
    taskData = taskData or taskUIItem.taskData
    local taskID = taskData.taskID or 0
    local taskConfig = CycleNightClubTaskManager:GetTaskConfigByTaskID(taskID)
    ...
    if taskConfig then
        ...
        local descStr,haveStr,needStr,isComplete = CycleNightClubTaskManager:GetTaskShowValues(taskID)
...
function CycleNightClubTaskUIView:Init()
    local taskData = CycleNightClubTaskManager:GetTaskData()
...
function CycleNightClubTaskUIView:RefreshAllTaskInfo()
    --1特殊处理,如果1完成了就显示1，没完成就找个完成的显示
    self:RefreshTaskInfo(self.m_taskUIItems[1],CycleNightClubTaskManager:GetType1TaskData())
...
---跳转到对应需要解锁的
function CycleNightClubTaskUIView:LocateRoom(roomID,furIndex)
        ...
        if curRoomData.state == 0 then
            ...
            self.m_currentModel:LookAtSceneGO(roomID,nil,nil,nil,function()
                ...
                if self.m_currentModel:RoomIsUnlock(unlock_room) then
                    CycleNightClubUnlockUI:ShowUI(roomID)
                end
            ...
            self.m_currentModel:LookAtSceneGO(roomID,furIndex,nil,nil,function()
                ...
                if curRoomCfg then
                    if curRoomCfg.room_category == 1 then
                        CycleNightClubBuildingUI:ShowFactoryUI(roomID,furIndex)
                    elseif curRoomCfg.room_category == 2 or curRoomCfg.room_category == 3 then
                        CycleNightClubBuildingUI:ShowSupplyBuildingUI(roomID,furIndex)
                    else
...
function CycleNightClubTaskUIView:LocateFurniture(taskConfig)
...
---跳转到任务目标
---@param taskUIItem TaskUIItem
function CycleNightClubTaskUIView:JumpToTaskTarget(taskUIItem)
    local taskConfig = taskUIItem.taskConfig
    if taskConfig then
        --CycleNightClubTaskManager:FakeSetTaskAsComplete(taskConfig.id)
        ...
        elseif taskConfig.task_type == 5 then
            self:DestroyModeUIObject(false,function()
                if not GameTableDefine.CycleNightClubSlotMachineUI.m_view then
        ...
        if taskConfig.task_type ~= 5 then
            if GameTableDefine.CycleNightClubSlotMachineUI.m_view then
                GameTableDefine.CycleNightClubSlotMachineUI:CloseView()
            end
        end
        self:DestroyModeUIObject()
...
return CycleNightClubTaskUIView