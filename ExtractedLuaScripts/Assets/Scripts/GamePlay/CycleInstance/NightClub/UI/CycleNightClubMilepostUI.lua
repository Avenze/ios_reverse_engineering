...
---@class CycleNightClubMilepostUI
...
function CycleNightClubMilepostUI:GetView()
    self.m_view = GameUIManager:SafeOpenUI(ENUM_GAME_UITYPE.CYCLE_NIGHT_CLUB_MILEPOST_UI, self.m_view, require("GamePlay.CycleInstance.NightClub.UI.CycleNightClubMilepostUIView"), self, self.CloseView)
    return self.m_view
end

function CycleNightClubMilepostUI:CloseView()
    GameUIManager:CloseUI(ENUM_GAME_UITYPE.CYCLE_NIGHT_CLUB_MILEPOST_UI)
    self.m_view = nil
    collectgarbage("collect")
end


function CycleNightClubMilepostUI:Refresh(scrollIndex,scrollToNewest)
...
function CycleNightClubMilepostUI:CalculateData()
    local data = {}
    local curData = {}
    ...
    for k, v in ipairs(curRewardCfg) do
        ...
        if shopCfg then
            ...
            printError("找不到config_shop的配置,id = "..shopID)
        end
        cur.info = {}
...
function CycleNightClubMilepostUI:GetRewardStatusByCfg(itemCfg)
    local result = 1
    --TODO:获取当前的里程碑经验
...
function CycleNightClubMilepostUI:ShowMilepostPanel(scrollIndex)
