...
---@class CycleNightClubSellUIView:UIBaseView
local CycleNightClubSellUIView = Class("CycleNightClubSellUIView", UIView)

function CycleNightClubSellUIView:ctor()
    self.super:ctor()
    self.m_data = {}
...
function CycleNightClubSellUIView:OnEnter()
    self.m_currentModel = CycleInstanceDataManager:GetCurrentModel()
    self.m_completeBlueprintGuide = self.m_currentModel:IsGuideCompleted(CycleInstanceDefine.GuideDefine.NightClub.UnlockBluePrint)
    ...
    self:SetButtonClickHandler(self:GetComp("choose/product/item/btn/upgrade_btn", "Button"), function()
        if self.m_completeBlueprintGuide then
            GameTableDefine.CycleNightClubBlueprintUI:OpenView(selectResID)
        else
...
function CycleNightClubSellUIView:OnExit()
...
function CycleNightClubSellUIView:ProductionSell()
    self.chooseGO:SetActive(false)
    ...
    playSelling = true
    GameTableDefine.CycleNightClubAIBlackBoard:RefreshShelf(workRoomData, selectResID)
    ...
    EventDispatcher:TriggerEvent(GameEventDefine.OnCycleProductionUnlock, selectResID, function()
        ...
        AnimationUtil.Play(animation,"UI_cy3_NightClubSellUI_price")
    end)
end

function CycleNightClubSellUIView:ShowWharfUI(roomID, select, isFirst, isBpUpgrade)
    if select then
        selectIndex = select
    else
        selectIndex = 1
...
function CycleNightClubSellUIView:ShowPortListItem(index,trans)
    index = index + 1
    ...
    local showRedPoint = CycleNightClubBluePrintManager:CanUpgradeProduction(resID,true) and self.m_completeBlueprintGuide
    ...
    local bluePrintMoneyBuff,bluePrintMileBuff = CycleNightClubBluePrintManager:GetProductBuffValue(resID)
    ...
    local data = CycleNightClubBluePrintManager:GetProductionDataByProductID(currentFurLevelCfg.resource_type)
    local level = data.starLevel or 0
    local bpConfig = CycleNightClubBluePrintManager:GetBPConfigByProductionLevel(currentFurLevelCfg.resource_type,level)
    ...
    self:CreateTimer(1, function()
        ...
        if isPort then
            if isUnlocked then
                if isSell then
                    self:SetSprite(self:GetComp(go, "product/selling/sellBox/nightClubIcon", "Image"), "UI_Common", bpConfig.icon)
                    ...
                    if selectIndex == index and playSelling then
                        animator:Play("Cy3_NightClubSellUI_Sell_unsaletosell")
                        playSelling = false
                    else
                        animator:Play("Cy3_NightClubSellUI_Sell_selling")
                    end
                else
                    self:SetText(go, "product/unsale/price/num", resSellPriceStr)
                    self:SetSprite(self:GetComp(go, "product/unsale/sellBox/nightClubIcon", "Image"), "UI_Common", bpConfig.icon)
                    ...
                    if not self.wharfFurData[tostring(index)].playUnlock then
                        animator:Play("Cy3_NightClubSellUI_Sell_unlocking")
                        self.wharfFurData[tostring(index)].playUnlock = true
                    else
                        animator:Play("Cy3_NightClubSellUI_Sell_unlock")
                    end
                end
            else
                animator:Play("Cy3_NightClubSellUI_Sell_lock")
            end
        end
    end, false)
...
function CycleNightClubSellUIView:SwitchSelect(index, cb)
...
function CycleNightClubSellUIView:SwitchSellData(roomID, index, furLevelID)
...
function CycleNightClubSellUIView:ShowChoose(selectGO, resSellPriceStr, bpConfig, isSell, isLocked)
    self.chooseGO:SetActive(false)
    ...
    self:SetSprite(self:GetComp("choose/product/item/Box/nightClubIcon", "Image"), "UI_Common", bpConfig.icon)
    self:SetText("choose/product/item/price/num", resSellPriceStr)
    local showRedPoint = CycleNightClubBluePrintManager:CanUpgradeProduction(bpConfig.res_id,true) and self.m_completeBlueprintGuide
...
function CycleNightClubSellUIView:GetFurnitureMaxlevel(furnitureID)
...
function CycleNightClubSellUIView:SelectFurniture(index, furData, resID, workRoomData, cb)
    --将相机焦点移动至所选设备位置
    local cameraFocus = self:GetComp("RootPanel/RoomTitle/SceneObjectRange", "CameraFocus")
    if selectIndex == index then
        GameTableDefine.CycleNightClubAIBlackBoard:RefreshShelf(workRoomData, resID)
        GameUIManager:GetEnableTouch(false)
...
---用来给引导返回最高售价的物品
function CycleNightClubSellUIView:GetBestSellGO()
...
function CycleNightClubSellUIView:GetLastSellGO()

    local portListScroll = self:GetComp("RootPanel/RoomInfo/List", "ScrollRectEx")
    local resCount = Tools:GetTableSize(resConfig)
    local lastRoomID = GameTableDefine.CycleNightClubBuildingUI.roomID
...
---用来给引导返回1号物品
function CycleNightClubSellUIView:GetFirstSellGO()
    local portListScroll = self:GetComp("RootPanel/RoomInfo/List", "ScrollRectEx")
...
---用来给引导返回1号物品Btn
function CycleNightClubSellUIView:GetFirstSellBtn()
    local portListScroll = self:GetComp("RootPanel/RoomInfo/List", "ScrollRectEx")
    local uiItemGO = portListScroll:GetScrollItem(0).gameObject
    return self:GetComp(uiItemGO,"product/unsale","Button")
end

function CycleNightClubSellUIView:GetGo(obj, child)
    ...
    elseif obj == "GetFirstSellRectGO" then
        ...
        return self:getSuper(CycleNightClubSellUIView).GetGo(self,obj, child)
    end
end

function CycleNightClubSellUIView:GetBestSellBtn()
...
function CycleNightClubSellUIView:GetComp(obj, child, uiType)
    if obj == "GetBestSellBtn" then
        return self:GetBestSellBtn()
    elseif obj == "GetFirstSellBtn" then
        return self:GetFirstSellBtn()
    else
        return self:getSuper(CycleNightClubSellUIView).GetComp(self,obj, child,uiType)
    end
end

return CycleNightClubSellUIView
