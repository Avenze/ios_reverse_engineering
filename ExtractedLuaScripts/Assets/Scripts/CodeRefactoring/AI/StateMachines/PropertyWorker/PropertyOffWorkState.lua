...
---@class PropertyOffWorkState:AIStateBase
---@field m_owner PropertyWorker
---下班
local PropertyOffWorkState = Class("PropertyOffWorkState",AIStateBase)
...
function PropertyOffWorkState:ctor()
...
function PropertyOffWorkState:OnEnter(...)
    self.m_isMoveTOBus = false
...
function PropertyOffWorkState:MoveToBus()
    ...
    local bus
    if self.m_owner:HasFlag(ActorDefine.Flag.FLAG_EMPLOYEE_MANAGER_WORKER) then
...
function PropertyOffWorkState:OnExit()
    if self.m_isMoveTOBus then
        self.m_isMoveTOBus = false
        self.m_targetBus:PassengerUnMoveToBus(self.m_owner)
        self.m_owner:StopMove()
...
function PropertyOffWorkState:OnDestroy()
    if self.m_timerID then
        Timer:StopTimer(self.m_timerID)
        self.m_timerID = nil
    end
...
function PropertyOffWorkState:Event(msg, params)
