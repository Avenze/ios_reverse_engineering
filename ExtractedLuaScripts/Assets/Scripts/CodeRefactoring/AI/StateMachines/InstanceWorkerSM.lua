--[[
    ...
    author:{author}
    time:2023-08-21 11:53:01
    description:{description}
]]
local AIStateMachine = require("CodeRefactoring.AI.AIStateMachine")
local Class = require("Framework.Lua.Class")
---@class InstanceWorkerSM:AIStateMachine
---@field super AIStateMachine
local InstanceWorkerSM = Class("InstanceWorkerSM", AIStateMachine)
...
function InstanceWorkerSM:Init( roomGO, furGO, spawnPos, workPosTr, faceTr, actionPosTr, ActionPosFaceTr)
    self.RoomGO = roomGO
    self.FurGO = furGO
...
function InstanceWorkerSM.InitStates(stateMachine)
    -- 初始化 根据当前条件查找下一个状态
...
function InstanceWorkerSM.OverwriteState(stateMachine)
    ----------------初始化状态---------------- 
...
function InstanceWorkerSM.TransState(stateMachine, stateType, ...)
    stateMachine.super.TransState(stateMachine, stateType, ...)
end

function InstanceWorkerSM.Destory(stateMachine)
...
--function InstanceWorkerSM.GetNearestHouse(stateMachine, roomType)
--    local actor = stateMachine.actor
